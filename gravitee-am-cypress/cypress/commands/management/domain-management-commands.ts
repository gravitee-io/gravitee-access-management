...
export const startDomain = (domain, accessToken) => {
    return requestDomain(domain, accessToken, "PATCH", {
        enabled: true
    }).should(response => expect(response.status).to.eq(200));
};

export const requestDomain = (domain, accessToken, method, body) => {
    return cy.request({
        url: domain ? getDomainManagerUrl() + `${domain}` : getDomainManagerUrl(),
        method: method,
...
export const createAcceptAllDeviceNotifier = (domain, accessToken) => {
    return cy.request({
        url: getDomainManagerUrl() + `${domain}/auth-device-notifiers`,
        method: 'POST',
        auth: { bearer: accessToken },
        body: {
