 ...
 *
 *   - Combiner branch reordering: anyOf/oneOf/allOf branches are compared positionally
 *     (index 0 to 0, etc.). Branches reordered between schemas will produce false positives.
 *
 *   - Conditional if-condition changes: when both schemas have if/then/else and the if condition
 *     changes, a WARN is emitted but the impact is not classified — which data satisfies the new
 *     condition cannot be determined without evaluating instances.
 *
 *   - not content changes: a WARN is emitted but the direction is not classified — a
 *     more-permissive not schema is actually more restrictive, making static analysis unreliable.
 *
...
function compareSchemas(oldSub, newSub, prefix) {
  ...
  if (newAllOf.length > oldAllOf.length) {
    ...
    for (let i = 0; i < compareLen; i++) {
      compareSchemas(oldAllOf[i], newAllOf[i], path(`allOf[${i}]`));
    }
  }
  ...
  if (!oldHasIf && newHasIf) {
    ...
    // if-condition change: which instances satisfy the condition cannot be determined statically
    if (!deepEqual(oldSub.if, newSub.if)) {
      warn(path('if'), `conditional if-condition changed — which instances are subject to then/else may differ; review manually`);
    }
    // then/else branch changes: recurse — tightening is breaking
    if (oldSub.then !== undefined && newSub.then !== undefined) {
      compareSchemas(oldSub.then, newSub.then, path('then'));
    } else if (oldSub.then === undefined && newSub.then !== undefined) {
      error(path('then'), `then branch added to existing conditional — data matching the if condition must now satisfy additional constraints`);
    } else if (oldSub.then !== undefined && newSub.then === undefined) {
      warn(path('then'), `then branch removed from conditional — previously enforced constraints no longer apply; verify intent`);
    }
    if (oldSub.else !== undefined && newSub.else !== undefined) {
      compareSchemas(oldSub.else, newSub.else, path('else'));
    } else if (oldSub.else === undefined && newSub.else !== undefined) {
      error(path('else'), `else branch added to existing conditional — data not matching the if condition must now satisfy additional constraints`);
    } else if (oldSub.else !== undefined && newSub.else === undefined) {
      warn(path('else'), `else branch removed from conditional — previously enforced constraints no longer apply; verify intent`);
    }
  }
...
function compareCombiners(oldList, newList, pathStr, keyword) {
  ...
  if (newLen < oldList.length) {
    ...
    for (let i = 0; i < oldList.length; i++) {
      compareSchemas(oldList[i], newList[i], `${pathStr}[${i}]`);
    }
  }
