...
public class MongoIdentityProviderRepository extends AbstractManagementMongoRepository implements IdentityProviderRepository {
    ...
    public Single<Set<IdentityProvider>> findAll(ReferenceType referenceType, String referenceId) {
        return Observable
            .fromPublisher(
                identitiesCollection.find(and(eq(FIELD_REFERENCE_TYPE, referenceType.name()), eq(FIELD_REFERENCE_ID, referenceId)))
            )
            .map(this::convert)
    ...
    public Maybe<IdentityProvider> findById(String identityProviderId) {
        return Observable
            .fromPublisher(identitiesCollection.find(eq(FIELD_ID, identityProviderId)).first())
            .firstElement()
            .map(this::convert);
    }
                ...
                identitiesCollection
                    .find(
                        and(
                            ...
                            eq(FIELD_ID, identityProviderId)
                        )
                    )
                    .first()
            )
    ...
    public Single<IdentityProvider> create(IdentityProvider item) {
        ...
        return Single
            .fromPublisher(identitiesCollection.insertOne(identityProvider))
            .flatMap(success -> findById(identityProvider.getId()).toSingle());
    }
    ...
    public Single<IdentityProvider> update(IdentityProvider item) {
        IdentityProviderMongo identityProvider = convert(item);
        return Single
            .fromPublisher(identitiesCollection.replaceOne(eq(FIELD_ID, identityProvider.getId()), identityProvider))
    ...
    private IdentityProviderMongo convert(IdentityProvider identityProvider) {
        ...
        identityProviderMongo.setMappers(
            identityProvider.getMappers() != null ? new Document((Map) identityProvider.getMappers()) : new Document()
        );
        identityProviderMongo.setRoleMapper(
            identityProvider.getRoleMapper() != null ? convert(identityProvider.getRoleMapper()) : new Document()
        );
