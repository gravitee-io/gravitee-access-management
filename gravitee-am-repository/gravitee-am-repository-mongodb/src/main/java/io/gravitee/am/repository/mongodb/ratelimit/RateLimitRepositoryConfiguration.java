...
public class RateLimitRepositoryConfiguration extends AbstractRepositoryConfiguration {
    ...
    public MongoDatabase mongoOperations() {
        final MongoClient mongo = connectionProvider.getClientWrapper(Scope.RATE_LIMIT.getName()).getClient();
        return mongo.getDatabase(getDatabaseName());
    }

    private String getDatabaseName() {
        boolean useManagementSettings = environment.getProperty(Scope.RATE_LIMIT.getRepositoryPropertyKey() + ".use-management-settings", Boolean.class, true);
        String propertyPrefix = useManagementSettings ? Scope.MANAGEMENT.getRepositoryPropertyKey() : Scope.RATE_LIMIT.getRepositoryPropertyKey();
