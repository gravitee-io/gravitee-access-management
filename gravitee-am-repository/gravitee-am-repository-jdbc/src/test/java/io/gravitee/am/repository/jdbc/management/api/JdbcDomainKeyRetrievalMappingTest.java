 ...
 * out of the SPIFFE block must read those limits back from the JSON already on disk.
 */
public class JdbcDomainKeyRetrievalMappingTest {
    ...
    public void shouldReadLegacySpiffeLimitsAsKeyRetrievalSettings() {
        KeyRetrievalSettings settings = JSONMapper.toBean(LEGACY_OIDC_JSON, OIDCSettings.class).getKeyRetrievalSettings();
    ...
    public void shouldKeepSpiffeValidationPolicyOnLegacyDocument() {
        OIDCSettings oidc = JSONMapper.toBean(LEGACY_OIDC_JSON, OIDCSettings.class);
    ...
    public void shouldPreferStoredKeyRetrievalSettingsOverLegacyLimits() {
        String json = """
                {"workloadIdentitySettings":{"fetchTimeoutMs":1234},"keyRetrievalSettings":{"fetchTimeoutMs":999}}""";

        assertEquals(999, JSONMapper.toBean(json, OIDCSettings.class).getKeyRetrievalSettings().getFetchTimeoutMs());
    }
    ...
    public void shouldRoundTripKeyRetrievalSettings() {
        ...
        KeyRetrievalSettings persisted = JSONMapper.toBean(JSONMapper.toJson(oidc), OIDCSettings.class)
