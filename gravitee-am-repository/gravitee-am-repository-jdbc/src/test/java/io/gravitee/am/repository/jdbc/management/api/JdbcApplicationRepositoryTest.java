 ...
 */
public class JdbcApplicationRepositoryTest extends AbstractManagementJdbcTest {
    ...
    public void testFindByDomainAndClientId() throws TechnicalException {
        ...
        testObserver.assertComplete();
        testObserver.assertNoErrors();
        testObserver.assertValue(app -> app.getId().equalsIgnoreCase(application.getId()));
    }
    ...
    private Application buildApplication() {
        ...
        app.setEnabled(true);
        app.setIdentities(Sets.newHashSet("ipd1"+random, "idp2"+random));
        app.setFactors(Sets.newHashSet("fact1"+random, "fact2"+random));
        ApplicationSettings settings = new ApplicationSettings();
