...
import io.gravitee.am.repository.jdbc.management.api.spring.SpringEntityStoreRepository;
 ...
 */
@Repository
public class JdbcEntityStoreRepository extends AbstractJdbcRepository implements EntityStoreRepository, InitializingBean {

    public static final String COL_ID = "id";
    public static final String COL_DOMAIN_ID = "domain_id";
    public static final String COL_NAME = "name";
    public static final String COL_LATEST_VERSION = "latest_version";
    ...
    @Autowired
    private SpringEntityStoreRepository entityStoreRepository;

    protected EntityStore toEntity(JdbcEntityStore entity) {
        return mapper.map(entity, EntityStore.class);
    }

    protected JdbcEntityStore toJdbcEntity(EntityStore entity) {
        return mapper.map(entity, JdbcEntityStore.class);
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        this.insertStatement = createInsertStatement("entity_stores", columns);
        this.updateStatement = createUpdateStatement("entity_stores", columns, List.of(COL_ID));
    }

    @Override
    public Maybe<EntityStore> findById(String id) {
        LOGGER.debug("findById({})", id);
        return this.entityStoreRepository.findById(id)
                .map(this::toEntity);
    }
    ...
    public Flowable<EntityStore> findByDomain(String domainId) {
        ...
        return this.entityStoreRepository.findByDomain(domainId)
                .map(this::toEntity);
    }
    ...
    public Maybe<EntityStore> findByDomainAndId(String domainId, String id) {
        ...
        return this.entityStoreRepository.findByDomainAndId(domainId, id)
                .map(this::toEntity);
    }
    ...
    public Single<EntityStore> create(EntityStore item) {
        ...
        LOGGER.debug("Create entityStore with id {}", item.getId());
        ...
        insertSpec = addQuotedField(insertSpec, COL_LATEST_VERSION, item.getLatestVersion(), Integer.class);
    ...
    public Single<EntityStore> update(EntityStore item) {
        LOGGER.debug("Update entityStore with id {}", item.getId());
        ...
        updateSpec = addQuotedField(updateSpec, COL_LATEST_VERSION, item.getLatestVersion(), Integer.class);
    ...
    public Completable delete(String id) {
        ...
        return this.entityStoreRepository.deleteById(id);
    }

    @Override
    public Completable deleteByDomain(String domainId) {
        LOGGER.debug("deleteByDomain({})", domainId);
        return monoToCompletable(getTemplate().delete(JdbcEntityStore.class)
