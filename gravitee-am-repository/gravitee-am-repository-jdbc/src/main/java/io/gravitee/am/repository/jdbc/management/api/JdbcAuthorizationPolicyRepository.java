...
import io.gravitee.am.repository.jdbc.management.api.spring.SpringAuthorizationPolicyRepository;
 ...
 */
@Repository
public class JdbcAuthorizationPolicyRepository extends AbstractJdbcRepository implements AuthorizationPolicyRepository, InitializingBean {
    ...
    @Autowired
    private SpringAuthorizationPolicyRepository authorizationPolicyRepository;

    protected AuthorizationPolicy toEntity(JdbcAuthorizationPolicy entity) {
        return mapper.map(entity, AuthorizationPolicy.class);
    }

    protected JdbcAuthorizationPolicy toJdbcEntity(AuthorizationPolicy entity) {
        return mapper.map(entity, JdbcAuthorizationPolicy.class);
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        this.insertStatement = createInsertStatement("authorization_policies", columns);
        this.updateStatement = createUpdateStatement("authorization_policies", columns, List.of(COL_ID));
    }

    @Override
    public Maybe<AuthorizationPolicy> findById(String id) {
        LOGGER.debug("findById({})", id);
        return this.authorizationPolicyRepository.findById(id)
                .map(this::toEntity);
    }
    ...
    public Flowable<AuthorizationPolicy> findByDomain(String domainId) {
        ...
        return this.authorizationPolicyRepository.findByDomain(domainId)
                .map(this::toEntity);
    }
    ...
    public Maybe<AuthorizationPolicy> findByDomainAndId(String domainId, String id) {
        ...
        return this.authorizationPolicyRepository.findByDomainAndId(domainId, id)
                .map(this::toEntity);
    }

    @Override
    public Flowable<AuthorizationPolicy> findByDomainAndEngineType(String domainId, String engineType) {
        LOGGER.debug("findByDomainAndEngineType({}, {})", domainId, engineType);
        return this.authorizationPolicyRepository.findByDomainAndEngineType(domainId, engineType)
                .map(this::toEntity);
    }
    ...
    public Single<AuthorizationPolicy> create(AuthorizationPolicy item) {
        ...
        LOGGER.debug("Create authorizationPolicy with id {}", item.getId());
    ...
    public Single<AuthorizationPolicy> update(AuthorizationPolicy item) {
        LOGGER.debug("Update authorizationPolicy with id {}", item.getId());
    ...
    public Completable delete(String id) {
        ...
        return this.authorizationPolicyRepository.deleteById(id);
    }

    @Override
    public Completable deleteByDomain(String domainId) {
        LOGGER.debug("deleteByDomain({})", domainId);
        return monoToCompletable(getTemplate().delete(JdbcAuthorizationPolicy.class)
