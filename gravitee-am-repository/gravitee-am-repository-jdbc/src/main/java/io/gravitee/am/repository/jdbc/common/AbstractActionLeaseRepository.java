 ...
 */
public abstract class AbstractActionLeaseRepository<T> extends AbstractJdbcRepository {

    protected abstract ActionLease toEntity(T entity);

    protected abstract T toJdbcEntity(ActionLease entity);

    protected abstract T buildJdbcActionLease(String action, String nodeId, LocalDateTime expiryDate);

    protected abstract Class<T> getJdbcEntityClass();

    protected Maybe<ActionLease> doAcquireLease(String action, String nodeId, Duration duration) {
        ...
        Mono<T> existingAction = getTemplate().selectOne(Query.query(where("action").is(action)), getJdbcEntityClass());

        Mono<T> atomicAcquire = getTemplate()
                .update(getJdbcEntityClass())
                ...
                .flatMap(updatedCount -> {
                    ...
                    T newLease = buildJdbcActionLease(action, nodeId, expiryDate);
    ...
    protected Completable doReleaseLease(String action, String nodeId) {
        ...
        return monoToCompletable(getTemplate().delete(getJdbcEntityClass()).matching(Query.query(whereClause)).all())
                .observeOn(Schedulers.computation());
    }
}
