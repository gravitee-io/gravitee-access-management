...
import io.gravitee.am.repository.jdbc.management.api.model.JdbcAuthorizationData;
import io.gravitee.am.repository.jdbc.management.api.spring.SpringAuthorizationDataRepository;
 ...
 */
@Repository
public class JdbcAuthorizationDataRepository extends AbstractJdbcRepository implements AuthorizationDataRepository, InitializingBean {
    ...
    @Autowired
    private SpringAuthorizationDataRepository authorizationDataRepository;

    protected AuthorizationData toEntity(JdbcAuthorizationData entity) {
        return mapper.map(entity, AuthorizationData.class);
    }

    protected JdbcAuthorizationData toJdbcEntity(AuthorizationData entity) {
        return mapper.map(entity, JdbcAuthorizationData.class);
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        this.insertStatement = createInsertStatement("authorization_data", columns);
        this.updateStatement = createUpdateStatement("authorization_data", columns, List.of(COL_ID));
    }

    @Override
    public Maybe<AuthorizationData> findById(String id) {
        LOGGER.debug("findById({})", id);
        return this.authorizationDataRepository.findById(id)
                .map(this::toEntity);
    }
    ...
    public Maybe<AuthorizationData> findByDomain(String domainId) {
        ...
        return this.authorizationDataRepository.findByDomain(domainId)
                .map(this::toEntity);
    }
    ...
    public Single<AuthorizationData> create(AuthorizationData item) {
        ...
        LOGGER.debug("Create authorizationData with id {}", item.getId());
    ...
    public Single<AuthorizationData> update(AuthorizationData item) {
        LOGGER.debug("Update authorizationData with id {}", item.getId());
    ...
    public Completable delete(String id) {
        ...
        return this.authorizationDataRepository.deleteById(id);
    }

    @Override
    public Completable deleteByDomain(String domainId) {
        LOGGER.debug("deleteByDomain({})", domainId);
        return monoToCompletable(getTemplate().delete(JdbcAuthorizationData.class)
