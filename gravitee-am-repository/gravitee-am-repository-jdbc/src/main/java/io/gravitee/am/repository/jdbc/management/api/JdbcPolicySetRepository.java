...
import io.gravitee.am.repository.jdbc.management.api.spring.SpringPolicySetRepository;
 ...
 */
@Repository
public class JdbcPolicySetRepository extends AbstractJdbcRepository implements PolicySetRepository, InitializingBean {
    ...
    @Autowired
    private SpringPolicySetRepository policySetRepository;

    protected PolicySet toEntity(JdbcPolicySet entity) {
        return mapper.map(entity, PolicySet.class);
    }

    protected JdbcPolicySet toJdbcEntity(PolicySet entity) {
        return mapper.map(entity, JdbcPolicySet.class);
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        this.insertStatement = createInsertStatement("policy_sets", columns);
        this.updateStatement = createUpdateStatement("policy_sets", columns, List.of(COL_ID));
    }

    @Override
    public Maybe<PolicySet> findById(String id) {
        LOGGER.debug("findById({})", id);
        return this.policySetRepository.findById(id)
                .map(this::toEntity);
    }
    ...
    public Flowable<PolicySet> findByDomain(String domainId) {
        ...
        return this.policySetRepository.findByDomain(domainId)
                .map(this::toEntity);
    }
    ...
    public Maybe<PolicySet> findByDomainAndId(String domainId, String id) {
        ...
        return this.policySetRepository.findByDomainAndId(domainId, id)
                .map(this::toEntity);
    }
    ...
    public Single<PolicySet> create(PolicySet item) {
        ...
        LOGGER.debug("Create policySet with id {}", item.getId());
    ...
    public Single<PolicySet> update(PolicySet item) {
        LOGGER.debug("Update policySet with id {}", item.getId());
    ...
    public Completable delete(String id) {
        ...
        return this.policySetRepository.deleteById(id);
    }

    @Override
    public Completable deleteByDomain(String domainId) {
        LOGGER.debug("deleteByDomain({})", domainId);
        return monoToCompletable(getTemplate().delete(JdbcPolicySet.class)
