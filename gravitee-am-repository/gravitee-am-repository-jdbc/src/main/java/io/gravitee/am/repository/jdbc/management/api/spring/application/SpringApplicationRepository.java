...
public interface SpringApplicationRepository extends RxJava2CrudRepository<JdbcApplication, String> {
    ...
    @Query("select count(a.id) from applications a where a.domain = :domain and ( a.name = :query or a.settings_client_id = :query)")
    Single<Long> countForStrictSearch(@Param("domain") String domain, @Param("query") String query);

    @Query("select count(a.id) from applications a where a.domain = :domain and ( a.name like :query or a.settings_client_id like :query)")
    Single<Long> countForWildCardSearch(@Param("domain") String domain, @Param("query") String query);
