...
public interface SpringUserRepository extends RxJava2CrudRepository<JdbcUser, String> {
    ...
    @Query("select count(u.id) from users u where u.reference_type = :refType and u.reference_id = :refId and " +
            "( u.username = :query OR u.email = :query OR u.additional_information_email = :query OR" +
            " u.display_name = :query OR u.first_name = :query or u.last_name = :query )")
    Single<Long> countForStrictSearch(@Param("refType")String refType, @Param("refId") String refId, @Param("query") String query);

    @Query("select count(u.id) from users u where u.reference_type = :refType and u.reference_id = :refId and " +
            "( u.username like :query OR u.email like :query OR u.additional_information_email like :query OR" +
            " u.display_name like :query OR u.first_name like :query or u.last_name like :query )")
    Single<Long> countForWildcardSearch(@Param("refType")String refType, @Param("refId") String refId, @Param("query") String query);
