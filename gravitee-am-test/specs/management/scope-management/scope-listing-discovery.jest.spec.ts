...
describe('Pagination', () => {
  ...
  it('should return all scopes with default pagination', async () => {
    ...
    expect(allScopes.totalCount).toBeGreaterThanOrEqual(SYSTEM_SCOPE_COUNT + PAGINATION_SCOPE_COUNT);
  ...
  it('should return last page with remaining items', async () => {
    const totalCount = SYSTEM_SCOPE_COUNT + PAGINATION_SCOPE_COUNT;
    const pageSize = 2;
