...
import { CbaFixture, generateExpiredCertificatePEM, generateUniqueCertificatePEM, setupFixture } from './fixture/cba-fixture';
...
beforeEach(async () => {
  user = await buildCreateAndTestUser(fixture.domain.id, fixture.accessToken, 123);
...
describe('Certificate Based Authentication - Enrollment', () => {
  it('should enroll a certificate for a user', async () => {
    ...
    const credential = await enrollCertificate(fixture.domain.id, user.id, fixture.accessToken, newCredential);
  ...
  it('should reject expired certificate', async () => {
    ...
    await expect(enrollCertificate(fixture.domain.id, user.id, fixture.accessToken, newCredential)).rejects.toMatchObject({
  ...
  it('should reject duplicate certificate', async () => {
    ...
    const firstCredential = await enrollCertificate(fixture.domain.id, user.id, fixture.accessToken, newCredential);
    ...
    await expect(enrollCertificate(fixture.domain.id, user.id, fixture.accessToken, duplicateCredential)).rejects.toMatchObject({
  ...
  it('should enforce certificate limit (20 per user)', async () => {
    ...
    const allCredentials = await listCertificateCredentials(fixture.domain.id, user.id, fixture.accessToken);
    ...
    await expect(enrollCertificate(fixture.domain.id, user.id, fixture.accessToken, limitExceededCredential)).rejects.toMatchObject({
      message: expect.stringContaining('400'),
    });

    // Verify the 21st certificate was NOT added
    const credentialsAfterLimit = await listCertificateCredentials(fixture.domain.id, user.id, fixture.accessToken);
...
describe('Certificate Credential Management', () => {
  it('should list certificate credentials for a user', async () => {
    ...
    const credential1 = await enrollCertificate(fixture.domain.id, user.id, fixture.accessToken, {
      certificatePem: cert1,
    });
    const credential2 = await enrollCertificate(fixture.domain.id, user.id, fixture.accessToken, {
      certificatePem: cert2,
    });

    // When: Admin lists certificates
    const credentials = await listCertificateCredentials(fixture.domain.id, user.id, fixture.accessToken);
  ...
  it('should get certificate credential by ID', async () => {
    ...
    const credential = await enrollCertificate(fixture.domain.id, user.id, fixture.accessToken, {
      certificatePem: certPem,
    });

    // When: Admin gets certificate by ID
    const retrieved = await getCertificateCredential(fixture.domain.id, user.id, credential.id, fixture.accessToken);
  ...
  it('should return 404 when getting non-existent certificate credential', async () => {
    ...
    await expect(getCertificateCredential(fixture.domain.id, user.id, nonExistentCredentialId, fixture.accessToken)).rejects.toMatchObject({
  ...
  it('should delete certificate credential', async () => {
    ...
    const beforeDelete = await getCertificateCredential(fixture.domain.id, user.id, credential.id, fixture.accessToken);
    expect(beforeDelete.id).toBe(credential.id);

    // When: Admin deletes certificate
    await deleteCertificateCredential(fixture.domain.id, user.id, credential.id, fixture.accessToken);

    // Then: Certificate is deleted and cannot be retrieved
    await expect(getCertificateCredential(fixture.domain.id, user.id, credential.id, fixture.accessToken)).rejects.toMatchObject({
      message: expect.stringContaining('404'),
    });
  ...
  it('should return 404 when deleting non-existent certificate credential', async () => {
    ...
    // Then: 404 error is returned
    await expect(
      deleteCertificateCredential(fixture.domain.id, user.id, nonExistentCredentialId, fixture.accessToken),
