...
export const setupApiManagementLoginSocialFixture = async (): Promise<ApiManagementLoginSocialFixture> => {
  ...
  if (!domain.id || !domain.hrid) throw new Error('Domain create did not return id/hrid');
  ...
  const appId = app.id!;
  const clientId = app.settings?.oauth?.clientId ?? 'my-client';
  const clientSecret = app.settings?.oauth?.clientSecret ?? 'my-client-secret';
  ...
  if (!newIdp) throw new Error('Create identity did not return id');
