...
import {
  createTokenExchangeMigrationFixture,
  TOKEN_EXCHANGE_REQUIREMENTS,
  TokenExchangeMigrationFixture,
} from './fixture/token-exchange-fixture';
import { currentChannel, describeWhen, itWhen } from './fixture/migration-versions';
import { setup } from '../test-fixture';

setup(120000);

const channelLabel = currentChannel();
const requires = TOKEN_EXCHANGE_REQUIREMENTS;
...
// A channel seeded by a version the trusted-issuer seed does not cover has no domains to assert against.
describeWhen(requires.seeded, channelLabel).each(getDataPlaneTargets())('migration token exchange [data plane $id]', (target) => {
  ...
  describe.each(TOKEN_EXCHANGE_VARIANTS)('trusted issuer seeded through the %s API', (variant: TokenExchangeVariant) => {
    ...
    itWhen(requires.managementApiReportsTrustedIssuer)('still reports the seeded trusted issuer on the security domain', async () => {
    ...
    itWhen(requires.managementApiServesTrustedDomains)('also exposes the trusted issuer as a trusted domain of its own', async () => {
    ...
    itWhen(requires.gatewayExchangesTokens)('exchanges a JWT minted by the trusted security domain for a local access token', async () => {
    ...
    itWhen(requires.gatewayExchangesTokens)('refuses a JWT that the trusted issuer did not sign', async () => {
      const issuerToken = await fixture.mintIssuerToken();
