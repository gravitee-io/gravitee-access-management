...
beforeAll(async () => {
    ...
    challengeRulePositiveCtx = new TestSuiteContext(domain, challengeRulePositiveClient, domain.domain.users[0], oidc.body.authorization_endpoint)
    challengeRuleNegativeCtx1 = new TestSuiteContext(domain, challengeRuleNegativeClient1, domain.domain.users[1], oidc.body.authorization_endpoint)
    challengeRuleNegativeCtx2 = new TestSuiteContext(domain, challengeRuleNegativeClient2, domain.domain.users[2], oidc.body.authorization_endpoint)
});
...
describe('When Challenge CONDITIONAL', () => {
    it('and rule evaluates to true should end MFA flow', async () => {
        ...
        const loginPostResponse = await postForm(action,
            {
                'X-XSRF-TOKEN': xsrf,
