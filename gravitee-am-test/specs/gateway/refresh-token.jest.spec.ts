...
beforeAll(async () => {
  ...
  const createdDomain = await createDomain(accessToken, uniqueName('refresh-token', true), faker.company.catchPhraseDescriptor());
  ...
  oidc = domainWithOidc.oidcConfig || (await getWellKnownOpenIdConfiguration(domain.hrid).expect(200)).body;
