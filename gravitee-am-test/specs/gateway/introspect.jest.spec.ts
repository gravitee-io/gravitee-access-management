...
import { deleteDomain, setupDomainForTest, waitForDomainSync } from '@management-commands/domain-management-commands';
...
beforeAll(async () => {
  ...
  const setup = await setupDomainForTest(uniqueName('introspect'), { accessToken, waitForStart: true });
  domain = setup.domain;
  oidc = setup.oidcConfig;
