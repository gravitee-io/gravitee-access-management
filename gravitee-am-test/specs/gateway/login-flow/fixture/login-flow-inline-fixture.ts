...
export interface LoginFlowInlineFixture extends Fixture {
  ...
  attemptLogin: (clientId: string, username: string, password: string) => Promise<any>;
}
...
export const setupInlineFixture = async (): Promise<LoginFlowInlineFixture> => {
  ...
  const inmemoryIdp2 = await createIdp(domain.id, accessToken, {
    ...
    configuration: JSON.stringify({
      users: [
        {
          firstname: 'my-user-2',
          lastname: 'my-user-lastname-2',
          username: 'user',
          password: '#CoMpL3X-P@SsW0Rd',
          email: 'inline-user-2@example.com',
        },
      ],
    }),
  ...
  return {
    ...
    appIdpPriority,
    appAccountTests,
    attemptLogin: async (clientId: string, username: string, password: string): Promise<any> => {
      const authResponse = await initiateLoginFlow(clientId, openIdConfiguration, domain!, 'code', REDIRECT_URI);
