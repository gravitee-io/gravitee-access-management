...
const expectedMsg = (setting) => {
    return selectSetting(setting).accountSettings.autoLoginAfterResetPassword ? selectSetting(setting).accountSettings.redirectUriAfterResetPassword : "success=reset_password_completed";
}
    ...
    describe("Gateway reset password", () => {
        ...
        describe("when a Password Policy has been configured", () => {
            ...
            if (selectedSetting.passwordSettings.excludeUserProfileInfoInPassword) {
                Object.entries(userProps).filter(([key]) => key !== 'password').forEach(([propName, value]) => {
