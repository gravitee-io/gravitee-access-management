...
beforeAll(async () => {
    ...
    domain = await createDomain(accessToken, 'redirect-uri-domain', 'redirect uri with EL')
...
describe('Redirect URI', () => {
    describe('Dynamic parameters', () => {
        ...
        it('Multi EL parameters', async () => {
            const authResponse = await initiateLoginFlow(applicationMultiELParam.settings.oauth.clientId, oidc, domain, 'code', multiParam.callback1);
        ...
        it('Multi EL parameters with regular one', async () => {
            const authResponse = await initiateLoginFlow(applicationMultiELParamAndRegular.settings.oauth.clientId, oidc, domain, 'code', multiParamAndRegular.callback1);
            const loginResponse = await login(authResponse, user.username, applicationMultiELParamAndRegular.settings.oauth.clientId, user.password);
            const tokenResponse = await performGet(loginResponse.headers['location'], '', {
                Cookie: loginResponse.headers['set-cookie'],
            }).expect(302);
        ...
        it('Failing EL evaluation', async () => {
            const authResponse = await initiateLoginFlow(applicationFailingELParam.settings.oauth.clientId, oidc, domain, 'code', failingELParam.callback1);
        ...
        it('EL uses nonexisting param - should evaluate to empty string', async () => {
            const authResponse = await initiateLoginFlow(applicationFailingELParam.settings.oauth.clientId, oidc, domain, 'code', failingELParam.callback2);
            const loginResponse = await login(authResponse, user.username, applicationFailingELParam.settings.oauth.clientId, user.password);
            const tokenResponse = await performGet(loginResponse.headers['location'], '', {
                Cookie: loginResponse.headers['set-cookie'],
            }).expect(302);
