...
beforeAll(async () => {
    ...
    application = await createApplication(domain.id, managementApiAccessToken, {
    ...
    }).then(app => updateApplication(domain.id, managementApiAccessToken, {
        "settings": {
            "oauth": {
        ...
        "identityProviders": [
            {"identity": idpSet.values().next().value.id, "priority": -1}
        ]
    }, app.id));
...
describe("SelfAccount - Change Password", () => {
    describe("Prepare - Sign in user", () => {
        it('must deliver an access_token', async () => {
            let response = await performPost(openIdConfiguration.token_endpoint, '', `grant_type=password&username=${user.username}&password=${user.password}`, {
...
describe("SelfAccount - Change Password", () => {
    describe("With default settings", () => {
        describe("End User", () => {
            it('must be able to change his password', async () => {
                ...
                await performPost(`${process.env.AM_GATEWAY_URL}/${domain.hrid}/account/api/changePassword`, '', `{"password": "${user.password}"}`, {
    ...
    describe("When oldPassword is requried", () => {
        ...
        describe("EndUser ", () => {
            it('must NOT be able to change his password if old one is missing', async () => {
                await performPost(`${process.env.AM_GATEWAY_URL}/${domain.hrid}/account/api/changePassword`, '', `{"password": "TestWithoutOldPassword"}`, {
                    'Content-type': 'application/json',
                    "Authorization": `Bearer ${endUserAccessToken}` 
                }).expect(400);
            });
            ...
            it('must NOT be able to change his password if old one is invalid', async () => {
                await performPost(`${process.env.AM_GATEWAY_URL}/${domain.hrid}/account/api/changePassword`, '', `{"password": "${user.password}", "oldPassword": "invalidpassword"}`, {
                    'Content-type': 'application/json',
                    "Authorization": `Bearer ${endUserAccessToken}` 
                }).expect(401);
            });
            ...
            it('must be able to change his password if old one is present', async () => {
                ...
                await performPost(`${process.env.AM_GATEWAY_URL}/${domain.hrid}/account/api/changePassword`, '', `{"password": "${user.password}", "oldPassword": "${oldPassword}"}`, {
                    'Content-type': 'application/json',
                    "Authorization": `Bearer ${endUserAccessToken}` 
                }).expect(204);
            });
    ...
    describe("When access_token is too old", () => {
        ...
        describe("EndUser ", () => {
            it('must be able to change his password if old one is present', async () => {
                let oldPassword =  user.password;
                await performPost(`${process.env.AM_GATEWAY_URL}/${domain.hrid}/account/api/changePassword`, '', `{"password": "fake", "oldPassword": "${oldPassword}"}`, {
                    'Content-type': 'application/json',
                    "Authorization": `Bearer ${endUserAccessToken}` 
                }).expect(401);
            });
