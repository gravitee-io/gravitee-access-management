...
describe('SAML metadata — expired org license', () => {
  it(jira`should return 404 when an expired org license is sent (treated same as no license) ${'AM-7238'}`, async () => {
    if (!fixture.hasExpiredLicense) {
      console.log('Skipping: AM_ORG_LICENSE_EXPIRED_PATH is not set — provide a path to an expired license file to enable this test');
      return;
    }
