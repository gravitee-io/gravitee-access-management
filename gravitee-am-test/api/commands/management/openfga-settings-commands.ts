...
const OPENFGA_SETTINGS_PATH = (domainId: string, engineId: string) =>
...
export async function getOpenFgaStore(domainId: string, engineId: string, accessToken: string): Promise<OpenFgaStore> {
  const response = await request(process.env.AM_MANAGEMENT_URL)
    .get(`${OPENFGA_SETTINGS_PATH(domainId, engineId)}/store`)
...
export async function createAuthorizationModel(
  ...
  const response = await request(process.env.AM_MANAGEMENT_URL)
    .post(`${OPENFGA_SETTINGS_PATH(domainId, engineId)}/authorization-models`)
    .set('Authorization', `Bearer ${accessToken}`)
    .send(model)
    .expect(201);
  return response.body;
...
export async function listAuthorizationModels(
  ...
  let path = `${OPENFGA_SETTINGS_PATH(domainId, engineId)}/authorization-models`;
  const params: string[] = [];
...
export async function addTuple(domainId: string, engineId: string, accessToken: string, tuple: Tuple): Promise<void> {
  await request(process.env.AM_MANAGEMENT_URL)
    .post(`${OPENFGA_SETTINGS_PATH(domainId, engineId)}/tuples`)
    .set('Authorization', `Bearer ${accessToken}`)
    .send(tuple)
    .expect(201);
}
...
export async function listTuples(
  ...
  let path = `${OPENFGA_SETTINGS_PATH(domainId, engineId)}/tuples`;
  const params: string[] = [];
...
export async function deleteTuple(domainId: string, engineId: string, accessToken: string, tuple: Tuple): Promise<void> {
  await request(process.env.AM_MANAGEMENT_URL)
    .delete(`${OPENFGA_SETTINGS_PATH(domainId, engineId)}/tuples`)
    .set('Authorization', `Bearer ${accessToken}`)
    .send(tuple)
    .expect(204);
}
...
export async function checkPermission(
  ...
  const response = await request(process.env.AM_MANAGEMENT_URL)
    .post(`${OPENFGA_SETTINGS_PATH(domainId, engineId)}/check`)
    .set('Authorization', `Bearer ${accessToken}`)
    .send(check)
    .expect(200);
  return response.body;
