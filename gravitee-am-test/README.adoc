...
- Optional environment vars (defaults provided): AM_MANAGEMENT_URL, AM_MANAGEMENT_ENDPOINT, AM_DEF_ORG_ID, AM_DEF_ENV_ID, AM_ADMIN_USERNAME, AM_ADMIN_PASSWORD.
...
+
----
cp scripts/provisioning/provision.example.json my-provision.json
----
. Run the provisioner:
+
----
