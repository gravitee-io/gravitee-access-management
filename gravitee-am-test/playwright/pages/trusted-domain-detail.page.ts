...
export class TrustedDomainDetailPage extends BasePage {
  async navigateToNew(domainId: string): Promise<void> {
    await this.navigate(`/environments/${this.envHrid}/domains/${domainId}/settings/trust-domains/new`);
  }

  async navigateToEdit(domainId: string, trustDomainId: string): Promise<void> {
    await this.navigate(`/environments/${this.envHrid}/domains/${domainId}/settings/trust-domains/${trustDomainId}`);
  }
  ...
  kindChoice(kind: TrustDomainKind): Locator {
    return this.page.locator(`[data-testid="kindChoice-${kind}"]`);
  }

  async chooseKind(kind: TrustDomainKind): Promise<void> {
    await this.kindChoice(kind).click();
    await this.waitForReady();
  }

  get selectedKind(): Locator {
    return this.page.locator('mat-radio-button.mat-mdc-radio-checked[data-testid^="kindChoice-"]');
  }

  get kindBadge(): Locator {
    return this.page.locator('[data-testid="trustDomainKind"]');
  }
  ...
  /* ------------------------------------------------------------------ */
  /*  Key material — one control for both kinds                          */
  /* ------------------------------------------------------------------ */
  ...
  /* ------------------------------------------------------------------ */
  /*  Scope mappings (token-exchange kind only)                          */
  /* ------------------------------------------------------------------ */
  ...
  /* ------------------------------------------------------------------ */
  /*  User binding (token-exchange kind only)                            */
  /* ------------------------------------------------------------------ */
