...
public class TwilioVerifyResourceProvider implements MFAResourceProvider {
    ...
    private HttpClientBuilder buildHttpClient() {
        ...
        if (this.httpClientProxyHttpsHost == null) {
            // proxy https not define, use the http one
            hostname = this.httpClientProxyHttpHost;
            port = this.httpClientProxyHttpPort;
            username = this.httpClientProxyHttpUsername;
            password = this.httpClientProxyHttpPassword;
        }
