...
for TABLE in "${TABLES[@]}"; do
  ...
  mariadb-dump -h $SOURCE_DB_HOST -P $SOURCE_DB_PORT -u $SOURCE_DB_USER -p$SOURCE_DB_PASSWORD --protocol=tcp --no-tablespaces --single-transaction --quick --no-create-info --lock-tables=false $SOURCE_DB_NAME $TABLE > /tmp/${TABLE}_backup.sql
  ...
  mariadb -h $DEST_DB_HOST -P $DEST_DB_PORT -u $DEST_DB_USER -p$DEST_DB_PASSWORD --protocol=tcp  $DEST_DB_NAME < /tmp/${TABLE}_backup.sql
