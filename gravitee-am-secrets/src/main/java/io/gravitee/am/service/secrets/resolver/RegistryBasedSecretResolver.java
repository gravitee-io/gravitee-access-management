...
public class RegistryBasedSecretResolver implements SecretResolver {
    ...
    private static Maybe<String> resolveSecretValue(SecretURL url, SecretProvider secretProvider) {
        return secretProvider.resolve(url)
            .flatMap(secretMap -> Maybe.fromOptional(secretMap.getSecret(url).map(Secret::asString)));
    }
}
