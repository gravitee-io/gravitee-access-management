...
public final class DefaultCodecs implements Codecs {
    ...
    private SqlServerType getServerType(Parameter parameter) {
        ...
        return of(parameter.getType().getJavaType());
    }

    /**
     * Resolve a {@link io.r2dbc.mssql.message.type.SqlServerType} by its {@code typeName}. Name comparison is case-insensitive.
     *
     * @param typeClazz the clazz type.
     * @return the resolved {@link io.r2dbc.mssql.message.type.SqlServerType}.
     * @throws IllegalArgumentException if the type name cannot be resolved to a {@link io.r2dbc.mssql.message.type.SqlServerType}
     */
    public static SqlServerType of(Class<?> typeClazz) {

        for (SqlServerType type : SqlServerType.values()) {
            final Class<?> javaType = type.getJavaType();
            if (javaType.equals(typeClazz) || javaType.equals(Byte.class) && Boolean.class.equals(typeClazz)) {
                return type;
            }
        }

        throw new IllegalArgumentException(String.format("Unknown type: %s", typeClazz.getName()));
    }
