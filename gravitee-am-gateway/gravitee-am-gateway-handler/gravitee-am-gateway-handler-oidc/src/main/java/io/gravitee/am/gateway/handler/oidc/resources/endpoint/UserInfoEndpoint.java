...
public class UserInfoEndpoint implements Handler<RoutingContext> {
    ...
    public void handle(RoutingContext context) {
        ...
        subjectManager.findUserBySub(accessToken)
                ...
                .map(user -> {
                    ...
                    return processCustomClaims(jwt, user, client, context);
                })
                // encode response
                .flatMap(jwt -> {
    ...
    private JWT processCustomClaims(JWT userInfoJwt,
        ...
        ExecutionContext executionContext = prepareContext(context, client, user);
        executionContextTokenEnhancer.enhanceToken(userInfoJwt, customClaims, executionContext);
        return userInfoJwt;
    }

    private ExecutionContext prepareContext(RoutingContext routingContext, Client client, User user) {
