...
public class StrategyGranterAdapter implements TokenGranter {
    ...
    private void handleGrantSpecificData(OAuth2Request oAuth2Request, TokenCreationRequest request) {
        ...
        switch (grantData) {
            ...
            case GrantData.AuthorizationCodeData data -> {
                oAuth2Request.setAuthorizationCode(data.authorizationCode());
            }
            ...
            case GrantData.UmaData data -> {
                oAuth2Request.setPermissions(data.permissions());
            }
            default -> {
