...
public class CibaTokenGranterTest {
    ...
    public void shouldRejectRequest_UnknownAuthReqId() {
        ...
        when(authenticationRequestService.retrieve(any(), any(), any())).thenReturn(Single.error(new AuthorizationRejectedException("unknown_req_id")));

        final TestObserver<Token> test = granter.grant(tokenRequest, mock(Client.class)).test();
        test.awaitDone(10, TimeUnit.SECONDS);
        test.assertError(AuthorizationRejectedException.class);
    }
    ...
    public void shouldRejectRequest_SlowDown() {
        ...
        when(tokenRequest.parameters()).thenReturn(value);
        when(authenticationRequestService.retrieve(any(), any(), any())).thenReturn(Single.error(new SlowDownException()));
