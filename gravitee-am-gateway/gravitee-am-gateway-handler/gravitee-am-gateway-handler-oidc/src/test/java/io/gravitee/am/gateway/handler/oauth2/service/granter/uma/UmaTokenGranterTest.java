...
public class UmaTokenGranterTest {
    ...
    public void setUp() {
        ...
        when(userAuthenticationManager.loadUserByUsername(USER_ID)).thenReturn(Maybe.just(user));
    ...
    public void grant_user_technicalException() {
        when(userAuthenticationManager.loadUserByUsername(USER_ID)).thenReturn(Maybe.error(TechnicalException::new));
        TestObserver<Token> testObserver = umaTokenGranter.grant(tokenRequest, client).test();
    ...
    public void grant_user_notFound() {
        when(userAuthenticationManager.loadUserByUsername(USER_ID)).thenReturn(Maybe.empty());
        TestObserver<Token> testObserver = umaTokenGranter.grant(tokenRequest, client).test();
        testObserver.assertNotComplete().assertError(UmaException.class).assertError(this::assertNeedInfo);
    }
