...
public class CibaAuthenticationRequestResolverTest {
    ...
    /** Wire the domain with a device-notifier id; the mock provider optionally implements IdentityProviderDependent. */
    private void stubNotifier(boolean identityProviderDependent) {
        ...
        AuthenticationDeviceNotifierProvider p = identityProviderDependent
                ? mock(AuthenticationDeviceNotifierProvider.class,
                       org.mockito.Mockito.withSettings().extraInterfaces(IdentityProviderDependent.class))
                : mock(AuthenticationDeviceNotifierProvider.class);
        when(deviceNotifierManager.getAuthDeviceNotifierProvider(eq("notifier-1"))).thenReturn(p);
    }
    ...
    public void rejectsUnknownHint_whenFederationDisabled() {
        // no notifier configured → provider not IdentityProviderDependent → federation disabled
    ...
    public void carriesHintAndSetsNoSubject_whenFederationEnabled() {
        ...
        TestObserver<CibaAuthenticationRequest> obs = resolver.resolve(requestWithLoginHint("acme|abc"), clientWithOpenid()).test();
        obs.awaitDone(10, TimeUnit.SECONDS);
        obs.assertNoErrors();
        obs.assertValue(r ->
                r.getSubject() == null
                && r.getUser() == null
                && "acme|abc".equals(r.getLoginHint()));
    }
    ...
    public void issSubJsonHint_withFederation_carriesVerbatim_withoutLocalLookup() {
        ...
        final String hint = "{\"format\":\"iss_sub\",\"iss\":\"https://op/\",\"sub\":\"acme|9\"}";
    ...
    // ── federated-hint routing tests ──────────────────────────────────

    @Test
    public void federatedHintResolution_enabledWhenProviderIsIdentityProviderDependent() {
        stubNotifier(true);
        when(userService.findByCriteria(any(FilterCriteria.class))).thenReturn(Single.just(List.of()));
    ...
    public void federatedHintResolution_disabledWhenNoNotifierConfigured() {
        // cibaSettings has no deviceNotifiers → provider not IdentityProviderDependent
        when(userService.findByCriteria(any(FilterCriteria.class))).thenReturn(Single.just(List.of()));
    ...
    @Test
    public void federatedHintResolution_disabledWhenProviderNotIdentityProviderDependent() {
        // notifier exists but does not implement IdentityProviderDependent
        stubNotifier(false);
        when(userService.findByCriteria(any(FilterCriteria.class))).thenReturn(Single.just(List.of()));
