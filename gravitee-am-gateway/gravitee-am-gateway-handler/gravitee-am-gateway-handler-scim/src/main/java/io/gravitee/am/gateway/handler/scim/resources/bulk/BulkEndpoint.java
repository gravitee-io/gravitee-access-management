...
public class BulkEndpoint {
    ...
    private Maybe<@NonNull BulkRequest> parseRequestBody(RoutingContext context) {
        return Maybe.fromSupplier(() -> {
            try {
                return context.body().asPojo(BulkRequest.class, configuration.getBulkMaxRequestLength());
            } catch (IllegalStateException e) {
                log.warn("The size of the bulk operation exceeds the maxPayloadSize ");
                throw TooManyOperationException.payloadLimitReached(configuration.getBulkMaxRequestLength());
            }
        });
    }
    ...
    private BulkRequest checkBulkRequest(BulkRequest bulkRequest) {
        ...
        if (bulkRequest.getOperations().size() > configuration.getBulkMaxRequestOperations()) {
            throw TooManyOperationException.tooManyOperation(configuration.getBulkMaxRequestOperations());
        }
