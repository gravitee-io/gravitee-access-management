...
public class GroupServiceImpl implements GroupService {
            ...
            .flatMap(
                groupPage -> {
                    ...
                    if (size <= 0) {
                        ...
                        return Observable
                            .fromIterable(groupPage.getData())
                            ...
                            .map(
                                groups ->
                                    new ListResponse<>(groups, groupPage.getCurrentPage() + 1, groupPage.getTotalCount(), groups.size())
                            );
                    }
                }
            )
                ...
                ex -> {
                    ...
                    return Single.error(
                        new TechnicalManagementException(
                            String.format("An error occurs while trying to find groups by domain %s", domain),
                            ex
                        )
                    );
                }
        ...
        return groupRepository
            ...
            .onErrorResumeNext(
                ex -> {
                    ...
                    return Maybe.error(
                        new TechnicalManagementException(
                            String.format("An error occurs while trying to find a user using its ID: %s", groupId),
                            ex
                        )
                    );
                }
    ...
    public Single<Group> create(Group group, String baseUrl) {
        ...
        return groupRepository
            .findByDomainAndName(domain.getId(), group.getDisplayName())
            .isEmpty()
            .map(
                isEmpty -> {
                    if (!isEmpty) {
                            ...
                            group1 -> {
                                ...
                                if (!existingGroup.getId().equals(group1.getId())) {
                                    throw new UniquenessException(
                                        "Group with display name [" + group.getDisplayName() + "] already exists"
                                    );
                                }
    ...
    private Single<Group> setMembers(Group group, String baseUrl) {
        ...
        if (members != null) {
            ...
            return userRepository
                .findByIdIn(memberIds)
                .map(
                    users -> {
                        List<Member> existingMembers = users
                            .stream()
                            .map(
                                user -> {
