...
public class BulkEndpointTest extends RxWebTestBase {
    ...
    public void should_reject_too_many_request() throws Exception {
        ...
        bulkRequest.setOperations(IntStream.range(0, bulkEndpointConfiguration.getBulkMaxRequestOperations() + 1).mapToObj(i -> bulkOp(POST, valueOf(i))).toList());
        ...
        testRequest(
                ...
                "{\n" +
                        "  \"status\" : \"413\",\n" +
                        "  \"detail\" : \"The bulk operation exceeds the maximum number of operations ("+bulkEndpointConfiguration.getBulkMaxRequestOperations()+").\",\n" +
    ...
    public void should_reject_too_large_request() throws Exception {
        ...
        final var bulkRequest = new BulkRequest();
        bulkRequest.setOperations(IntStream.range(0, bulkEndpointConfiguration.getBulkMaxRequestOperations() - 1).mapToObj(i -> bulkOp(POST, valueOf(i), largePayload)).toList());
        bulkRequest.setSchemas(of(BULK_REQUEST_SCHEMA));
        testRequest(
                HttpMethod.POST,
                "/Bulk",
                ...
                "{\n" +
                        "  \"status\" : \"413\",\n" +
                        "  \"detail\" : \"The size of the bulk operation exceeds the maxPayloadSize ("+bulkEndpointConfiguration.getBulkMaxRequestLength()+").\",\n" +
                        "  \"schemas\" : [ \"urn:ietf:params:scim:api:messages:2.0:Error\" ]\n" +
                        "}");
