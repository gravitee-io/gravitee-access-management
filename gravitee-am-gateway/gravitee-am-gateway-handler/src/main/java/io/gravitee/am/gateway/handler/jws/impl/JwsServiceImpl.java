 ...
 */
package io.gravitee.am.gateway.handler.jws.impl;
...
public class JwsServiceImpl implements JwsService {
    ...
    public JWSVerifier verifier(JWK jwk) {
        try {
            switch (KeyType.valueOf(jwk.getKty())) {
                ...
                default:
                    throw new InvalidClientException("Assertion is using and unknown/not managed algorithm");
            }
        }catch (IllegalArgumentException | NotImplementedException ex) {
            throw new InvalidClientException("Assertion is using and unknown/not managed algorithm: "+ex.getMessage());
        }
    }
    ...
    private JWSVerifier from(RSAKey rsaKey) {
        try {
            ...
            LOGGER.error(ex.getMessage(),ex);
            throw new InvalidClientException("Assertion is using and unknown/not managed key");
        }
    }
    ...
    private JWSVerifier from(ECKey ecKey) {
        try {
            ...
            if(!curve.isPresent()) {
                throw new InvalidClientException("Unknown EC Curve: "+ecKey.getCrv());
            }
