...
const checkPassword = () => {
    ...
    if (isResetForm && passwordSettings && passwordValidation) {
        ...
        formData.append('password', passwordInput.value);
        passwordPromise = fetch(passwordValidation, {
            method: 'POST',
            body: formData
        });
    }
...
function enableSubmitButton(){
    Promise.all([checkPasswordHistory(), checkPassword()]).then((values) => {
        const responseHistory = values[0];
        const responseValidation = values[1];

        const passwordNotInHistory = responseHistory ? responseHistory.ok : true;
        if (excludePasswordsInHistory) {
            validateMessageElement(excludePasswordsInHistory, passwordNotInHistory);
        }

        const validPassword = responseValidation ? responseValidation.ok : true;
        if (excludeUserProfileInfoInPassword) {
            validateMessageElement(excludeUserProfileInfoInPassword, validPassword);
        }
        if (passwordNotInHistory && validPassword) {
