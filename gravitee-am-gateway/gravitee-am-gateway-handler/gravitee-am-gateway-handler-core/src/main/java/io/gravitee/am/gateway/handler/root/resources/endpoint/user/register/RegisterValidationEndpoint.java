...
import static io.gravitee.am.model.Template.REGISTRATION_VALIDATION;

/**
 * @author Titouan COMPIEGNE (titouan.compiegne at graviteesource.com)
 * @author GraviteeSource Team
 */
public class RegisterValidationEndpoint extends AbstractEndpoint implements Handler<RoutingContext> {

    private static final Logger logger = LoggerFactory.getLogger(RegisterValidationEndpoint.class);
    ...
    public RegisterValidationEndpoint(TemplateEngine engine, Domain domain,
    ...
    public void handle(RoutingContext routingContext) {
        ...
        if (!routingContext.request().params().contains("error") && !routingContext.request().params().contains("success")) {
            ...
            user.getAdditionalInformation().put("registration_code", IntStream.range(0, 6).map(i -> new Random().nextInt(10)).mapToObj(Integer::toString).collect(Collectors.joining()));
            userService.update(user)
                    .subscribe(updatedUser -> {
                        emailService.send(REGISTRATION_VALIDATION, user, client);
                    });
        }

        // render the registration confirmation page
        this.renderPage(routingContext, data, client, logger, "Unable to render registration validation page");
    }
