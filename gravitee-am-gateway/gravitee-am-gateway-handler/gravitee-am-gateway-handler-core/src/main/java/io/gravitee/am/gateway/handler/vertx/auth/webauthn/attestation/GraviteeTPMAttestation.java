...
//This class is to remove when fix by Vertx will be merged and new version of Vertx is released. https://github.com/eclipse-vertx/vertx-auth/issues/684
public class GraviteeTPMAttestation implements Attestation {
    ...
    public AttestationCertificates validate(WebAuthnOptions options, MetaData metadata, byte[] clientDataJSON, JsonObject attestation, AuthData authData) throws AttestationException {
        ...
        try {
            ...
            ASN extension = ASN1.parseASN1(subjectAltName);
            ...
            ASN root = ASN1.parseASN1(extension.binary(0));
            ...
            for (int i = 0; i < set.length(); i++) {
                ASN el = set.object(i);
                ASN oid = el.object(0, OBJECT_IDENTIFIER);
                ASN val = el.object(1, UTF8_STRING);
            ...
            for (int i = 0; i < root.length(); i++) {
                ASN el = root.object(i, OBJECT_IDENTIFIER);
