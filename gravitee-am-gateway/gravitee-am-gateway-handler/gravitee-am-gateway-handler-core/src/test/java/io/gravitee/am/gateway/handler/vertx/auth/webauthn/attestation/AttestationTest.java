...
public class AttestationTest {
    ...
    public void testU2FAttestation() {
        ...
        webAuthN.authenticate(
                new WebAuthn4JCredentials(new JsonObject()
                    ...
                    // process should not fail for an encoding issue
                    if (u.failed()) {
    ...
    private static CredentialStorage noopCredentialStorage() {
        return new CredentialStorage() {
            @Override
            public io.vertx.core.Future<java.util.List<io.vertx.ext.auth.webauthn4j.Authenticator>> find(String username, String credentialId) {
                return io.vertx.core.Future.succeededFuture(java.util.Collections.emptyList());
            }
            @Override
            public io.vertx.core.Future<Void> storeCredential(io.vertx.ext.auth.webauthn4j.Authenticator authenticator) {
                return io.vertx.core.Future.succeededFuture();
            }
            @Override
            public io.vertx.core.Future<Void> updateCounter(io.vertx.ext.auth.webauthn4j.Authenticator authenticator) {
                return io.vertx.core.Future.succeededFuture();
            }
        };
    }
