...
public class WebAuthnLoginHandlerTest extends RxWebTestBase {
    ...
    public void shouldNotLogin_v1_deviceIntegrityException() throws Exception {
        ...
        when(userAuthenticationManager.connectPreAuthenticatedUser(any(), any(), any())).thenReturn(Single.just(new User()));

        router.route(HttpMethod.POST, "/webauthn/login")
                .handler(rc -> {
                    Client client = new Client();
                    rc.session().put(ConstantKeys.PASSWORDLESS_CHALLENGE_KEY, "KidEI7a_CXoW7dKjb8JEnL_WUb2xDPWBHtCUzTo4ouAyKNzc0SbGmC0t8mh0cKHdqo_4hYc3c2Iewp_QF746rA");
    ...
    private WebAuthn webAuthn() {
        RepositoryCredentialStore credentialStore = mock(RepositoryCredentialStore.class);
        when(credentialStore.store(any())).thenReturn(Completable.complete());
        Authenticator authenticator = new Authenticator();
        authenticator.setCredID("QAn8i7wToriAx38jzUrVw04Cxao_y285vX2CGwyFtO8");
        authenticator.setPublicKey("pQECAyYgASFYILO4CFUFksL1no2vpWhmHMplz47TI9X8Wbt4UK936appIlggRrlbIdCLVdwBVkgdmYLVIJ0p2Zonb6fmha5vb-oXnro");
        when(credentialStore.fetch(any())).thenReturn(Single.just(Collections.singletonList(authenticator)));

        RelyingParty relyingParty = new RelyingParty();
        relyingParty
                .setName("Gravitee.io Access Management")
                .setId(RequestUtils.getDomain("http://localhost:8092"));

        return WebAuthn.create(
                        vertx, new GraviteeWebAuthnOptions().setRelyingParty(relyingParty))
