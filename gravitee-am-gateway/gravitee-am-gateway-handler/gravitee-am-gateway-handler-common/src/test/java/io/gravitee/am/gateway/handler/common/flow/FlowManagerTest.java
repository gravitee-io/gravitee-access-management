...
public class FlowManagerTest {
    ...
    public void shouldNoFindByExtensionPoint_noFlows() {
        ...
        when(flowService.findAll(ReferenceType.DOMAIN, domain.getId())).thenReturn(Single.just(Collections.emptyList()));
    ...
    public void shouldNotFindByExtensionPoint_flowDisabled() {
        ...
        when(domain.getId()).thenReturn("domain-id");
        when(flowService.findAll(ReferenceType.DOMAIN, domain.getId())).thenReturn(Single.just(Collections.singletonList(flow)));
        flowManager.afterPropertiesSet();
        TestObserver<List<Policy>> obs = flowManager.findByExtensionPoint(ExtensionPoint.PRE_CONSENT).test();
        obs.awaitTerminalEvent();
        obs.assertValue(policies -> {
    ...
    public void shouldFindByExtensionPoint_twoFlows_inherit_false() {
        ...
        when(flowService.findAll(ReferenceType.DOMAIN, domain.getId())).thenReturn(Single.just(Arrays.asList(domainFlow, appFlow)));
        flowManager.afterPropertiesSet();
        TestObserver<List<Policy>> obs = flowManager.findByExtensionPoint(ExtensionPoint.PRE_CONSENT, client).test();
        obs.awaitTerminalEvent();
        obs.assertValue(policies -> {
