...
public class MFAChallengeStep extends MFAStep {
    ...
    public void execute(RoutingContext routingContext, AuthenticationFlowChain flow) {
        ...
        if (context.isUserSilentAuth()) {
            log.debug("User '{}' silently authenticated, skip challenge", context.getEndUserId());
            stopMfaFlow(context, flow);
            return;
        }
        if (!isMfaFlowStopped(context)) {
            if (context.isUserSelectedEnrollFactor()) {
                log.debug("User '{}' is enrolling factor, execute Challenge", context.getEndUserId());
                ...
                if (!context.isChallengeCompleted() && stepUpRequired) {
                    log.debug("User '{}' needs to be challenged (stepUp={})", context.getEndUserId(), stepUpRequired);
                    challenge(context, flow);
    ...
    private void required(MfaFilterContext context, AuthenticationFlowChain flow) {
        if (context.isUserStronglyAuth() || isRememberDevice(context)) {
            log.debug("User '{}' already strongly authenticated or device is remembered", context.getEndUserId());
            continueFlow(context, flow);
        } else {
            log.debug("Challenge required for user '{}' (deviceAlreadyExists={})", context.getEndUserId(), context.deviceAlreadyExists());
            challenge(context, flow);
        }
    }
    ...
    private void conditional(MfaFilterContext context, AuthenticationFlowChain flow, Client client) {
        if (challengeConditionSatisfied(client, context, ruleEngine)) {
            log.debug("Challenge condition satisfied for user '{}', skip challenge", context.getEndUserId());
            continueFlow(context, flow);
        } else if (context.getRememberDeviceSettings().isSkipChallengeWhenRememberDevice()) {
            log.debug("Challenge condition not satisfied for user '{}', check remembered device if enabled", context.getEndUserId());
            required(context, flow);
        } else if (context.isUserStronglyAuth()) {
            log.debug("Challenge condition not satisfied for user '{}' but already strongly authenticate, skip challenge", context.getEndUserId());
            continueFlow(context, flow);
        } else {
            log.debug("Challenge condition not satisfied for user '{}', execute challenge", context.getEndUserId());
            challenge(context, flow);
        }
    }
    ...
    private void riskBased(MfaFilterContext context, AuthenticationFlowChain flow, Client client) {
        if (context.isUserStronglyAuth() || isSafe(context, client)) {
            log.debug("RiskBased evaluate user '{}' sign-in as safe, skip challenge", context.getEndUserId());
            continueFlow(context, flow);
        } else {
            log.debug("RiskBased evaluate user '{}' sign-in as unsafe, execute challenge", context.getEndUserId());
            challenge(context, flow);
        }
    }
