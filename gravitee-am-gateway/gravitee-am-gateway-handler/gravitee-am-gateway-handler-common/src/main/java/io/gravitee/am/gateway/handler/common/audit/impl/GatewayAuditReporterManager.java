...
public class GatewayAuditReporterManager extends AbstractService<AuditReporterManager> implements AuditReporterManager, EventListener<ReporterEvent, Payload>, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        deployment.subscribe(id -> {
            ...
            // Start reporters
            getEnvironment()
                    .doOnSuccess(env -> this.organizationId = env.getOrganizationId())
                    ...
                    .subscribe(tupleReportersContext -> {
                                if (!tupleReportersContext.getT1().isEmpty()) {
                                    tupleReportersContext.getT1()
                                            .forEach(reporter -> startReporterProvider(reporter, tupleReportersContext.getT2()));
                ...
                .subscribe(
                        tupleReporterContext -> {
                            ...
                            if (reporterPlugins.containsKey(reporterId)) {
                                loaded = updateReporterProvider(tupleReporterContext.getT1(), tupleReporterContext.getT2());
                            } else {
                                loaded = startReporterProvider(tupleReporterContext.getT1(), tupleReporterContext.getT2());
                            }
                            if(loaded) {
                                logger.info("Reporter {} {}d for domain {}", reporterId, eventType, domain.getName());
                            }
