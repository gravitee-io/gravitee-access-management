...
public class ScheduledPurgeService extends AbstractService implements Runnable {
    ...
    @Value("${repositories.gateway.type:mongodb}")
    private String gatewayRepositoryType;

    @Value("${repositories.oauth2.type:mongodb}")
    private String oAuth2repositoryType;
    ...
    protected void doStart() throws Exception {
        if (enabled && jdbcEnabled()) { // even if enabled, useless for Mongo implementation
            super.doStart();
    ...
    private boolean jdbcEnabled(){
        return "jdbc".equalsIgnoreCase(gatewayRepositoryType) || "jdbc".equalsIgnoreCase(oAuth2repositoryType) ;
    }
