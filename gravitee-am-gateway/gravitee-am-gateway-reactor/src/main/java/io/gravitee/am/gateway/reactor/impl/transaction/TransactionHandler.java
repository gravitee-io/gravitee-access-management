...
public class TransactionHandler implements Handler<RoutingContext> {
    ...
    public void handle(RoutingContext context) {
        ...
        updateSession(context, transactionId);
        context.next();
    }

    private void updateSession(RoutingContext routingContext, String transactionId) {
        Session session = routingContext.session();
        if(session != null && session.get(ConstantKeys.TRANSACTION_ID_KEY) == null) {
