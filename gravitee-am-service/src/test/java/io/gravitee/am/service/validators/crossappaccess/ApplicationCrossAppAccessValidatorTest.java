...
class ApplicationCrossAppAccessValidatorTest {
    ...
    @Test
    void shouldIgnoreIdJagValidityWhenDisabled() {
        assertThat(validator.validate(oauthSettings(false, 0, mapping("rs-1", "client-1")))).isEmpty();
    }

    @Test
    void shouldRejectMalformedResourceServersWhenDisabled() {
        assertThat(validator.validate(oauthSettings(false, 0, mapping("rs-1", ""))))
                .hasValue("crossAppAccessSettings.resourceServers entries must have a non-blank clientId");
    }

    @Test
    void shouldRejectNullResourceServerEntryWhenDisabled() {
        ApplicationOAuthSettings settings = oauthSettings(false, 0);
        settings.getCrossAppAccessSettings().setResourceServers(new ArrayList<>(Collections.singletonList(null)));
