...
public class CompositeDictionaryProvider implements DictionaryProvider {
    ...
    public Properties getDictionaryFor(Locale locale) {
        if (locale != null) {
            // get secondary properties first and then override them using the primary ones
            var properties = this.secondaryProvider.getDictionaryFor(locale);
            properties.putAll(this.mainProvider.getDictionaryFor(locale));
            return properties;
        }
