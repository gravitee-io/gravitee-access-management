...
public class CertificateServiceImpl implements CertificateService {
                ...
                ex -> {
                    ...
                    return Maybe.error(
                        new TechnicalManagementException(
                            String.format("An error occurs while trying to find a certificate using its ID: %s", id),
                            ex
                        )
                    );
                }
        ...
        return certificateRepository
            ...
            .onErrorResumeNext(
                ex -> {
                    ...
                    return Single.error(
                        new TechnicalManagementException("An error occurs while trying to find certificates by domain", ex)
                    );
                }
            );
    }
        ...
        return certificateRepository
            ...
            .onErrorResumeNext(
                ex -> {
                    ...
                    return Single.error(
                        new TechnicalManagementException("An error occurs while trying to find all certificates by domain", ex)
                    );
                }
            );
    }
    ...
    public Single<Certificate> create(String domain, NewCertificate newCertificate, User principal) {
        ...
        Single<Certificate> certificateSingle = certificatePluginService
            ...
            .flatMapSingle(
                new Function<CertificateSchema, SingleSource<Certificate>>() {
                    @Override
                                            ...
                                            key -> {
                                                try {
                                                    ...
                                                    newCertificate.setConfiguration(
                                                        objectMapper.writeValueAsString(certificateConfiguration)
                                                    );
        ...
        return certificateSingle
            ...
            .flatMap(
                certificate -> {
                    Event event = new Event(
                        Type.CERTIFICATE,
                        new Payload(certificate.getId(), ReferenceType.DOMAIN, certificate.getDomain(), Action.CREATE)
                    );
            ...
            .doOnSuccess(
                certificate -> {
                    ...
                    auditService.report(
                        AuditBuilder
                            .builder(CertificateAuditBuilder.class)
                            .principal(principal)
    ...
    public Single<Certificate> update(String domain, String id, UpdateCertificate updateCertificate, User principal) {
        ...
        return certificateRepository
            ...
            .flatMapSingle(
                new Function<Certificate, SingleSource<CertificateWithSchema>>() {
                    @Override
                    public SingleSource<CertificateWithSchema> apply(Certificate certificate) throws Exception {
                        return certificatePluginService
                            ...
                            .flatMapSingle(
                                new Function<String, SingleSource<? extends CertificateWithSchema>>() {
                                    @Override
                                    public SingleSource<? extends CertificateWithSchema> apply(String schema) throws Exception {
                                        return Single.just(
                                            new CertificateWithSchema(certificate, objectMapper.readValue(schema, CertificateSchema.class))
                                        );
                                    }
                                }
                            );
            ...
            .flatMap(
                oldCertificate -> {
                    Single<Certificate> certificateSingle = Single.create(
                        emitter -> {
                            ...
                            try {
                                ...
                                JsonNode oldCertificateConfiguration = objectMapper.readTree(
                                    oldCertificate.getCertificate().getConfiguration()
                                );
                                        ...
                                        key -> {
                                            try {
                                                ...
                                                if (!oldFileInformation.equals(fileInformation)) {
                                                    JsonNode file = objectMapper.readTree(certificateConfiguration.get(key).asText());
                                                    byte[] data = Base64.getDecoder().decode(file.get("content").asText());
                                                    certificateToUpdate.setMetadata(
                                                        Collections.singletonMap(CertificateMetadata.FILE, data)
                                                    );

                                                    // update configuration to set the file path
                                                    ((ObjectNode) certificateConfiguration).put(key, file.get("name").asText());
                                                    updateCertificate.setConfiguration(
                                                        objectMapper.writeValueAsString(certificateConfiguration)
                                                    );
                                                }
                                            } catch (IOException ex) {
                    ...
                    return certificateSingle
                        ...
                        .flatMap(
                            certificate1 -> {
                                Event event = new Event(
                                    Type.CERTIFICATE,
                                    new Payload(certificate1.getId(), ReferenceType.DOMAIN, certificate1.getDomain(), Action.UPDATE)
                                );
    ...
    public Completable delete(String certificateId, User principal) {
        ...
        return certificateRepository
            ...
            .flatMapSingle(
                certificate ->
                    applicationService
                        .findByCertificate(certificateId)
                        .flatMap(
                            applications -> {
            ...
            .flatMapCompletable(
                certificate -> {
                    ...
                    Event event = new Event(
                        Type.CERTIFICATE,
                        new Payload(certificate.getId(), ReferenceType.DOMAIN, certificate.getDomain(), Action.DELETE)
                    );
                    return certificateRepository
                            ...
                            () ->
                                auditService.report(
                                    AuditBuilder
                                        .builder(CertificateAuditBuilder.class)
                                        .principal(principal)
                                        .type(EventType.CERTIFICATE_DELETED)
                                        .certificate(certificate)
                                )
                        )
                        .doOnError(
            ...
            .onErrorResumeNext(
                ex -> {
                    ...
                    return Completable.error(
                        new TechnicalManagementException(
                            String.format("An error occurs while trying to delete certificate: %s", certificateId),
                            ex
                        )
                    );
                }
    ...
    public Single<Certificate> create(String domain) {
        ...
        return certificatePluginService
            ...
            .getSchema(certificate.getType())
            .map(
                new Function<String, CertificateSchema>() {
                    @Override
            ...
            .map(
                new Function<CertificateSchema, String>() {
                    @Override
                    public String apply(CertificateSchema certificateSchema) throws Exception {
                        ...
                        final String sigAlgName = environment.getProperty(
                        ...
                        final String storePass = environment.getProperty(
            ...
            .flatMapSingle(
                new Function<String, SingleSource<Certificate>>() {
                    @Override
