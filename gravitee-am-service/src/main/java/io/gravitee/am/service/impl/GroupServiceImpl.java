...
public class GroupServiceImpl implements GroupService {
    ...
    public Maybe<Group> findByName(ReferenceType referenceType, String referenceId, String groupName) {
        ...
        return groupRepository
            .findByName(referenceType, referenceId, groupName)
            .onErrorResumeNext(
                ex -> {
                    LOGGER.error(
                        "An error occurs while trying to find a group using its name: {} for the {} {}",
                        groupName,
                        referenceType,
                        referenceId,
                        ex
                    ...
                    return Maybe.error(
                        new TechnicalManagementException(
                            String.format(
                                "An error occurs while trying to find a user using its name: %s for the %s %s",
                                groupName,
                                referenceType,
                                referenceId
                            ),
        ...
        return groupRepository
            ...
            .onErrorResumeNext(
                ex -> {
                    ...
                    return Single.error(
                        new TechnicalManagementException(
                            String.format("An error occurs while trying to find a user using member: %s", memberId),
                            ex
                        )
                    );
                }
        ...
        return groupRepository
            ...
            .onErrorResumeNext(
                ex -> {
                    ...
                    return Maybe.error(
                        new TechnicalManagementException(
                            String.format("An error occurs while trying to find a group using its id: %s", id),
                            ex
                        )
                    );
                }
            ...
            .flatMap(
                group -> {
                    if (group.getMembers() == null || group.getMembers().isEmpty()) {
                        ...
                        List<String> pagedMemberIds = sortedMembers.subList(
                            Math.min(sortedMembers.size(), page),
                            Math.min(sortedMembers.size(), page + size)
                        );
        ...
        return groupRepository
            ...
            .onErrorResumeNext(
                ex -> {
                    ...
                    return Single.error(
                        new TechnicalManagementException(
                            String.format("An error occurs while trying to find a group using ids: %s", ids),
                            ex
                        )
                    );
                }
    ...
    public Single<Group> create(
        ...
        return findByName(referenceType, referenceId, newGroup.getName())
            ...
            .flatMap(
                group -> {
                    Event event = new Event(
                        Type.GROUP,
                        new Payload(group.getId(), group.getReferenceType(), group.getReferenceId(), Action.CREATE)
                    );
            ...
            .doOnSuccess(
                group ->
                    auditService.report(
                        AuditBuilder.builder(GroupAuditBuilder.class).principal(principal).type(EventType.GROUP_CREATED).group(group)
                    )
            )
            .doOnError(
                throwable ->
                    auditService.report(
                        AuditBuilder
                            .builder(GroupAuditBuilder.class)
                            .principal(principal)
    ...
    public Single<Group> update(
        ...
        return findById(referenceType, referenceId, id)
            ...
            .flatMapMaybe(
                existingGroup ->
                    groupRepository
                        .findByName(referenceType, referenceId, updateGroup.getName())
                        .map(Optional::of)
                    ...
                    return setMembers(groupToUpdate)
                        ...
                        .flatMap(
                            group -> {
                                Event event = new Event(
                                    Type.GROUP,
                                    new Payload(group.getId(), group.getReferenceType(), group.getReferenceId(), Action.UPDATE)
                                );
                                return eventService.create(event).flatMap(__ -> Single.just(group));
                            }
                        )
                        ...
                        .andThen(
                            Completable.fromSingle(
                                eventService.create(
                                    new Event(
                                        Type.DOMAIN,
                                        new Payload(group.getId(), group.getReferenceType(), group.getReferenceId(), Action.DELETE)
                                    )
                                )
                            )
                        )
            ...
            .onErrorResumeNext(
                ex -> {
                    ...
                    return Completable.error(
                        new TechnicalManagementException(String.format("An error occurs while trying to delete group: %s", groupId), ex)
                    );
                }
            );
    }
    ...
    @Override
    public Single<Group> assignRoles(
        ReferenceType referenceType,
        String referenceId,
        String groupId,
        List<String> roles,
    ...
    @Override
    public Single<Group> revokeRoles(
        ReferenceType referenceType,
        String referenceId,
        String groupId,
        List<String> roles,
    ...
    private Single<Group> assignRoles0(
        ReferenceType referenceType,
        String referenceId,
        String groupId,
        List<String> roles,
                            ...
                            group1 ->
                                auditService.report(
                                    AuditBuilder
                                        .builder(GroupAuditBuilder.class)
                                        .principal(principal)
                                        .type(EventType.GROUP_ROLES_ASSIGNED)
                                        .oldValue(oldGroup)
                                        .group(group1)
                                )
                        )
    ...
    private Single<Group> setMembers(Group group) {
        List<String> userMembers = group.getMembers() != null
            ? group.getMembers().stream().filter(Objects::nonNull).distinct().collect(Collectors.toList())
            : null;
        if (userMembers != null && !userMembers.isEmpty()) {
            return userService
                .findByIdIn(userMembers)
                .map(
                    users -> {
