...
public class ProtectedResourceServiceImpl implements ProtectedResourceService {
    ...
    public Single<ProtectedResourceSecret> create(Domain domain, User principal, NewProtectedResource newProtectedResource) {
        ...
        toCreate.setUpdatedAt(toCreate.getUpdatedAt());
        ...
        toCreate.setClientSecrets(List.of(hashClientSecret(domain, secretSettings, rawSecret)));

        return oAuthClientRegistry.checkClientIdUniqueness(domain.getId(), toCreate.getClientId())
    ...
    private ClientSecret hashClientSecret(Domain domain, ApplicationSecretSettings secretSettings, String rawSecret) {
