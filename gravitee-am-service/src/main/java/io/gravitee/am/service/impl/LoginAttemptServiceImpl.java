...
public class LoginAttemptServiceImpl implements LoginAttemptService {
                ...
                optionalLoginAttempt -> {
                    if (optionalLoginAttempt.isPresent()) {
                        ...
                        if (loginAttempt.getAttempts() >= accountSettings.getMaxLoginAttempts()) {
                            loginAttempt.setExpireAt(
                                new Date(System.currentTimeMillis() + (accountSettings.getAccountBlockedDuration() * 1000))
                            );
                        }
                        ...
                        if (loginAttempt.getAttempts() >= accountSettings.getMaxLoginAttempts()) {
                            ...
                            loginAttempt.setExpireAt(
                                new Date(System.currentTimeMillis() + (accountSettings.getLoginAttemptsResetTime() * 1000))
                            );
                        }
                ...
                ex -> {
                    ...
                    return Completable.error(
                        new TechnicalManagementException(
                            String.format("An error occurs while trying to delete login attempt: %s", criteria),
                            ex
                        )
                    );
                }
        ...
        return loginAttemptRepository
            ...
            .onErrorResumeNext(
                ex -> {
                    ...
                    return Maybe.error(
                        new TechnicalManagementException(
                            String.format("An error occurs while trying to fin login attempt by id: %s", id),
                            ex
                        )
                    );
                }
    ...
    private Completable lockAccount(LoginAttemptCriteria criteria, AccountSettings accountSettings) {
        return userService
            .findByDomainAndUsernameAndSource(criteria.domain(), criteria.username(), criteria.identityProvider())
