...
public class PasswordHistoryRepositoryTest extends AbstractDataPlaneTest {
    ...
    public void shouldFindPasswordHistoryForUser() {
        ...
        var testObserver = repository.findUserHistory(Reference.domain(REF_ID), userId).toList().test();
        testObserver.awaitDone(10, TimeUnit.SECONDS);
        testObserver.assertComplete();
        testObserver.assertNoErrors();
    ...
    public void shouldFindPasswordHistoryForDomain() {
        ...
        var testSubscriber = repository.findByReference(Reference.domain(REF_ID)).test();
    ...
    public void shouldDeletePasswordHistoryForDomain() {
        ...
        var testObserver = repository.deleteByReference(Reference.domain(REF_ID)).test();
        testObserver.awaitDone(10, TimeUnit.SECONDS);
        testObserver.assertComplete();
        testObserver.assertNoErrors();
        var testSubscriber = repository.findByReference(Reference.domain(REF_ID)).test();
    ...
    public void shouldDeletePasswordHistoryForUser() {
        ...
        testObserver.assertComplete();
        testObserver.assertNoErrors();
        var testSubscriber = repository.findUserHistory(Reference.domain(REF_ID), userId).test();
        testSubscriber.awaitDone(10, TimeUnit.SECONDS);
        testSubscriber.assertComplete();
        testSubscriber.assertNoErrors();
        testSubscriber.assertValueCount(0);
