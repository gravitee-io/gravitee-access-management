 ...
 */
@Repository
public interface SpringUserRepository extends RxJava3CrudRepository<JdbcUser, String> {
    ...
    @Query("select u.* from users u left join user_identities ui on ui.user_id = u.id where u.reference_type = :refType and u.reference_id = :refId and UPPER(ui.username) = UPPER(:name) and ui.provider_id = :src ")
