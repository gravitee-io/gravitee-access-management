...
import io.gravitee.am.dataplane.mongodb.repository.model.uma.ResourceMongo;
 ...
 */
@Component
public class MongoResourceRepository extends AbstractDataPlaneMongoRepository implements ResourceRepository {
    ...
    @PostConstruct
    public void init() {
        resourceCollection = mongoDatabase.getCollection(COLLECTION_NAME, ResourceMongo.class);
        MongoUtils.init(resourceCollection);
    ...
    public Flowable<Resource> findByDomain(String domain) {
        return Flowable.fromPublisher(resourceCollection.find(eq(FIELD_DOMAIN, domain)).sort(new BasicDBObject(FIELD_UPDATED_AT, -1))).map(this::convert)
                .observeOn(Schedulers.computation());
    }
