...
import io.gravitee.am.dataplane.exceptions.DataPlaneTechnicalException;
import io.gravitee.am.dataplane.mongodb.repository.model.GroupMongo;
 ...
 */
@Component
public class MongoGroupRepository extends AbstractDataPlaneMongoRepository implements GroupRepository {
    ...
    @PostConstruct
    public void init() {
        groupsCollection = mongoDatabase.getCollection("groups", GroupMongo.class);
    ...
    public Single<Page<Group>> searchScim(ReferenceType referenceType, String referenceId, FilterCriteria criteria, int offset, int size) {
        try {
            ...
            return Single.error(new DataPlaneTechnicalException("An error has occurred while searching groups with filter criteria", ex));
        }
    }
