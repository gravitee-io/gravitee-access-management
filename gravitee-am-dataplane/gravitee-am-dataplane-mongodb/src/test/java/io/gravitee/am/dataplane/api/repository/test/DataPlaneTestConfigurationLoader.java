 ...
 */
@Configuration
@ComponentScan({"io.gravitee.am.dataplane.mongodb.provider", "io.gravitee.am.dataplane.mongodb.repository"})
public class DataPlaneTestConfigurationLoader {

    @Bean
    public MongodbProvider embeddedClient() {
        return new MongodbProvider("test-am-management");
    }

    @Bean(name = "dataPlaneMongoDatabase")
    public MongoDatabase mongoOperations() {
        return embeddedClient().getMongoDatabase();
    }
