...
/**
 * Purpose of this simulation is to sign in users only with login/password using the code flow and execute an extension grant flow
 *
 * Possible arguments:
 ...
 */
class MultiDomainExtensionGrantFlow extends Simulation {

  val httpProtocol = http
    .userAgentHeader("Gatling - Basic Login and Extension Grant Flow")
    .disableFollowRedirect

  val userGenerator = userFeeder(WORKLOAD)
  val domainGenerator = multiDomainsFeeder(WORKLOAD)

  val scn = scenario("Basic Login and Extension Grant Flow")
