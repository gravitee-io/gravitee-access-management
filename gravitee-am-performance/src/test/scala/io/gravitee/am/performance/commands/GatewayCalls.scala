...
object GatewayCalls {
  ...
  def renderLoginForm() = {
    http("Get Login Form")
      //.get(GATEWAY_BASE_URL + s"/#{domainName}/login?client_id=${APP_NAME}&response_type=code&redirect_uri=https://callback-${APP_NAME}")
      .get("#{" + NEXT_ACTION_KEY + "}")
      .check(status.is(200))
  ...
  def submitLoginForm() = {
    http("Post Login Form")
      //.post(GATEWAY_BASE_URL + s"/#{domainName}/login?client_id=${APP_NAME}&response_type=code&redirect_uri=https://callback-${APP_NAME}")
