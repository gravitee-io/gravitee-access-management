...
class CreateMultipleDomains extends Simulation {
  // TODO create a simulation to create role / groups and assign them to users on existing domain
  // TODO create simulation to create additional users on domains
  // TODO in login flow with consent allow to specify scopes as parameters (always provide openId so we can do the UserInfo call)
