...
import io.gravitee.am.gateway.handler.common.policy.UserActionPolicyException;
...
public class ExampleUserActionPolicy extends AbstractUserActionPolicy {
    ...
    private static final String ERROR_KEY = "EXAMPLE_USER_ACTION_POLICY_ERROR";
     ...
     */
    @Override
    public Completable performUserAction(Request request, ExecutionContext context) {
        ...
        if (requestParam == null || requestParam.equals("error")) {
            return Completable.error(new UserActionPolicyException("error bad parameter from custom user action policy", ERROR_KEY));
        }
