...
class AuthorizationPollerTest {
    ...
    CibaClient auth0; GatewayCallbackClient callback; PendingAuthStore store; AuthorizationPoller poller;
    ...
    void pending_keeps_record() {
        when(auth0.pollToken("R1")).thenReturn(Single.just(new CibaClient.PollResult(CibaClient.PollKind.PENDING, null, null, null, "authorization_pending")));
    ...
    void token_fires_validated_true_when_witness_matches() {
        ...
        when(auth0.pollToken("R1")).thenReturn(Single.just(new CibaClient.PollResult(
    ...
    void error_fires_validated_false_and_clears() {
        when(auth0.pollToken("R1")).thenReturn(Single.just(new CibaClient.PollResult(CibaClient.PollKind.ERROR, null, null, null, "access_denied")));
        assertEquals(AuthorizationPoller.Outcome.DENIED, poller.pollOnce("tid1").blockingGet());
        verify(callback).postCallback(eq(CB_URL), eq("stateJwt"), eq("tid1"), eq(false), nullable(String.class), nullable(String.class));
        assertNull(store.get("tid1"));
        assertFalse(poller.hasClient("tid1"), "clientsByTid must be cleaned on DENIED");
    ...
    void schedule_error_in_poll_cleans_client_and_store() throws Exception {
        ...
        when(auth0.pollToken("R1")).thenReturn(Single.error(new RuntimeException("upstream failure")));
        Vertx vertx = Vertx.vertx();
        try {
            latchPoller.schedule(vertx, "tid1", 0 /* effectively 1 ms */, auth0);
