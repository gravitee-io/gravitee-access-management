...
class CibaClientTest {
    ...
    // a vendor-neutral RFC 9396 authorization_details sample (same "fdx_v1.0" convention used by the sibling tests)
    ...
    void bc_authorize_relays_login_hint_verbatim() {
        ...
        var client = new CibaClient(webClient, resolver,
                "http://localhost:" + wm.port() + "/.well-known/openid-configuration", "cid", "sec", "https://api", "client_secret_post");
    ...
    void bc_authorize_relays_login_hint_token_verbatim() {
        ...
        var client = new CibaClient(webClient, resolver,
                "http://localhost:" + wm.port() + "/.well-known/openid-configuration", "cid", "sec", null, "client_secret_post");
    ...
    void builds_client_from_connection_fields_and_polls_against_bundle_host() {
        ...
        var c = new CibaClient(webClient, resolver, conn.wellKnownUri(), conn.clientId(), conn.clientSecret(), "https://api.example", "client_secret_post");
        wm.stubFor(post(urlEqualTo("/oauth/token"))
    ...
    void basic_auth_sends_authorization_header_and_no_secret_in_body() {
        ...
        wm.stubFor(post(urlEqualTo("/bc-authorize"))
                .willReturn(okJson("{\"auth_req_id\":\"R1\",\"expires_in\":120,\"interval\":5}")));
        var client = new CibaClient(webClient, resolver,
                "http://localhost:" + wm.port() + "/.well-known/openid-configuration", "cid", "sec", null, "client_secret_basic");
        client.bcAuthorize(hint, null, "openid", null, null).blockingGet();
        wm.verify(postRequestedFor(urlEqualTo("/bc-authorize"))
    ...
    void poll_basic_auth_sends_authorization_header_and_no_secret_in_body() {
        var resolver = resolverFor();
        wm.stubFor(post(urlEqualTo("/oauth/token"))
                .willReturn(okJson("{\"access_token\":\"AT\"}")));
        var client = new CibaClient(webClient, resolver,
                "http://localhost:" + wm.port() + "/.well-known/openid-configuration", "cid", "sec", null, "client_secret_basic");
        client.pollToken("R1").blockingGet();
        wm.verify(postRequestedFor(urlEqualTo("/oauth/token"))
                .withHeader("Authorization", matching("Basic .+"))
                .withRequestBody(containing("client_id=cid"))
                .withRequestBody(notMatching(".*client_secret=.*")));
    ...
    void unsupported_client_auth_method_fails_closed_at_build() {
        var resolver = resolverFor();
        var ex = assertThrows(IllegalStateException.class, () -> new CibaClient(webClient, resolver,
                "http://localhost:" + wm.port() + "/.well-known/openid-configuration", "cid", "sec", null, "private_key_jwt"));
