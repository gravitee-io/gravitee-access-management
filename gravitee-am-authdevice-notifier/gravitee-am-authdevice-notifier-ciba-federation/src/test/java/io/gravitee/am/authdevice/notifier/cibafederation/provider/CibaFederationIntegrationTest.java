...
class CibaFederationIntegrationTest {
    ...
    void full_flow_approved() {
        ...
        // Any relayed RAR exercises this poll/callback/cross-witness plumbing test; use the
        // vendor-neutral passthrough so relayed == inbound and the witness echo stays exact.
        var rendered = new PassthroughRelayStrategy().relay(fdx);
    ...
    void provider_notify_poll_callback_relays_verbatim_hint_and_carries_id_token() {
        ...
        var provider = CibaFederationAuthenticationDeviceNotifierProvider.forTest(
                (conn, aud) -> a0, new PassthroughRelayStrategy(), store, poller, vertx, 120);
    ...
    /**
     * Same provider->poll->callback chain as above, but driven with the Auth0 user-profile transform
     * strategy ({@link Auth0UserProfileRelayStrategy}), not passthrough. Proves the cross-witness
    ...
    void provider_notify_poll_callback_relays_transformed_user_profile_rar_and_matches_cross_witness() {
        ...
        acmeAuth.stubFor(post(urlEqualTo("/oauth/token")).inScenario("e2e-transform").whenScenarioStateIs("Started")
                .willReturn(aResponse().withStatus(400).withBody("{\"error\":\"authorization_pending\"}"))
                .willSetStateTo("ok"));
        // access_token is an unsigned JWT carrying sub=acme|10; authorization_details echoes the RENDERED URN.
        // id_token is a small unsigned JWT carrying its own sub — it is what the callback forwards (asserted below).
        ...
        // Auth0 user-profile transform strategy — relay renders FDX to the user-profile URN.
        var provider = CibaFederationAuthenticationDeviceNotifierProvider.forTest(
                (conn, aud) -> a0, new Auth0UserProfileRelayStrategy(recipient), store, poller, vertx, 120);
