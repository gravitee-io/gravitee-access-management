...
import { ClientRegistrationDefaultScopeComponent } from './default-scope.component';

describe('ClientRegistrationDefaultScopeComponent', () => {
  const domain = {
    id: 'domain-id',
    name: 'Test domain',
    oidc: {
  ...
  let component: ClientRegistrationDefaultScopeComponent;
  ...
  beforeEach(() => {
    ...
    component = new ClientRegistrationDefaultScopeComponent(
      domainService,
      {} as ActivatedRoute,
  ...
  it('should preserve configured scopes and synchronize the domain store after saving', () => {
    component.patch();

    expect(domainService.patchOpenidDCRSettings).toHaveBeenCalledWith('domain-id', {
      oidc: { clientRegistrationSettings: { defaultScopes: ['openid'] } },
    });
