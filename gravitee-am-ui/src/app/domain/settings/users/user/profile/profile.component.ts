...
export class UserProfileComponent implements OnInit {
  ...
  resendConfirmationRegistration(event) {
    event.preventDefault();
    this.dialogService.confirm('Send Email', 'Are you sure you want to send the confirmation registration email ?').subscribe((res) => {
      if (res) {
  ...
  resetPassword() {
    this.dialogService.confirm('Reset Password', 'Are you sure you want to reset the password ?').subscribe((res) => {
      if (res) {
        this.userService.resetPassword(this.domainId, this.user.id, this.password).subscribe(() => {
          ...
          for (const name of Object.keys(this.passwordForm.controls)) {
  ...
  unlock(event) {
    event.preventDefault();
    this.dialogService.confirm('Unlock User', 'Are you sure you want to unlock the user ?').subscribe((res) => {
      if (res) {
