...
export class GroupRolesComponent implements OnInit {
  ...
  add() {
    let dialogRef = this.dialog.open(AddGroupRolesComponent, {
      width: '700px',
  ...
  revoke(event, role) {
    ...
    this.dialogService.confirm('Revoke role', 'Are you sure you want to remove role from this group ?').subscribe((res) => {
      if (res) {
