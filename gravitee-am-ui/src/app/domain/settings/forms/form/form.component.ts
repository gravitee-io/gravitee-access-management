...
export class FormComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    if (this.form && this.form.content) {
      ...
      this.originalFormContent = (' ' + this.originalFormContent).slice(1);
      this.formFound = true;
    } else {
      this.form = {};
  ...
  create() {
    ...
    this.formService.create(this.domainId, this.form).map(res => res.json()).subscribe(data => {
  ...
  update() {
    this.form['content'] = this.formContent;
    this.formService.update(this.domainId, this.form.id, this.form).map(res => res.json()).subscribe(data => {
  ...
  delete(event) {
    ...
    this.dialogService
      ...
      .subscribe(res => {
        if (res) {
          this.formService.delete(this.domainId, this.form.id).subscribe(response => {
