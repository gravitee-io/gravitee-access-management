...
export class OpenFGAComponent implements OnInit, OnDestroy {
  ...
  loadStoreInfo() {
    this.subscriptions.add(
      this.openFGAService.getStore(this.domainId, this.engineId).subscribe({
        next: (store) => {
          this.storeInfo = store;
        },
        error: () => {
          this.snackbarService.open('Failed to load store information');
        },
      }),
    );
  }
  ...
  loadAuthorizationModel() {
    this.subscriptions.add(
      this.openFGAService.getAuthorizationModel(this.domainId, this.engineId).subscribe({
        next: (response) => {
          if (response && response.authorizationModel) {
            try {
              // Store JSON model
              this.authorizationModelJson = response.authorizationModel;

              // Transform to DSL for display
              this.authorizationModelDsl = transformer.transformJSONStringToDSL(JSON.stringify(response.authorizationModel));
              ...
              this.authorizationModelDsl = response.authorizationModel;
            }
          } else {
  ...
  loadTuples() {
    ...
    this.subscriptions.add(
      this.openFGAService.listTuples(this.domainId, this.engineId).subscribe({
        next: (tuples) => {
          this.tuples = tuples || [];
  ...
  saveConfiguration() {
    ...
    this.subscriptions.add(
      this.authorizationEngineService.update(this.domainId, this.engineId, updatedEngine).subscribe({
        ...
        error: () => {
          this.snackbarService.open('Failed to save configuration');
        },
      }),
    );
  }
