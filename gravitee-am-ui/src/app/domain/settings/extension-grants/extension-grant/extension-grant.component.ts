...
const CROSS_APP_ACCESS_EXTENSION_GRANT = 'cross-app-access-am-extension-grant';
...
export class ExtensionGrantComponent implements OnInit {
  ...
  update() {
    ...
    this.extensionGrantService.update(this.domainId, this.extensionGrant.id, this.extensionGrant).subscribe(() => {
  ...
  delete(event) {
    ...
    this.dialogService
      ...
      .pipe(
        filter((res) => res),
        switchMap(() => this.extensionGrantService.delete(this.domainId, this.extensionGrant.id)),
        tap(() => {
