...
export class TrustDomainFormComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    this.tokenExchangeEnabled = !!this.trustDomain?.tokenExchangeEnabled || (this.createMode && !this.spiffeEnabled);
  ...
  get oidcEnabled(): boolean {
    return this.tokenExchangeEnabled || this.crossAppAccessEnabled;
  }
  ...
  get keySource(): string {
    return this.model?.keyMaterial?.source ?? DEFAULT_KEY_MATERIAL_SOURCE;
  }
  ...
  getValidationErrors(): string[] {
    ...
    if (!this.tokenExchangeEnabled && !this.crossAppAccessEnabled && !this.spiffeEnabled) {
      errors.push('Pick at least one usage: Token exchange, Cross App Access, SPIFFE, or several of them.');
    }
  ...
  private issuerErrors(): string[] {
    const issuer = (this.model.issuer ?? '').trim();
    if (!issuer) {
      return ['Issuer URL is required.'];
    }
    if (issuer.length > ISSUER_MAX_LENGTH) {
      return [`The issuer URL must be at most ${ISSUER_MAX_LENGTH} characters.`];
    }
    return this.crossAppAccessEnabled && !isAbsoluteUri(issuer)
      ? [`Issuer URL must be an absolute URI when Cross App Access is enabled: ${issuer}`]
      : [];
  }
  ...
  private payload(): TrustDomain {
    ...
    return {
      ...
      issuer: this.oidcEnabled ? (this.model.issuer ?? '').trim() : '',
      tokenExchangeEnabled: this.tokenExchangeEnabled,
  ...
  private loadDomainScopesIfNeeded(): void {
    if ((this.tokenExchangeEnabled || this.crossAppAccessEnabled) && !this.domainScopes.length) {
