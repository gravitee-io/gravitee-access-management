...
@Component({
  ...
  styleUrls: ['./policy-set.component.scss'],
  standalone: false,
})
export class PolicySetComponent implements OnInit {
  ...
  loadLatestVersion() {
    this.policySetService.getVersion(this.domainId, this.policySet.id, this.policySet.latestVersion).subscribe((v) => {
      this.content = v.content || '';
    });
  }

  loadVersions() {
    this.policySetService.getVersions(this.domainId, this.policySet.id).subscribe((versions) => {
  ...
  update() {
    ...
    this.policySetService.update(this.domainId, this.policySet.id, updatePayload).subscribe((data) => {
      this.policySet = data;
      this.formChanged = false;
      this.commitMessage = '';
      this.loadLatestVersion();
      this.loadVersions();
      this.snackbarService.open('Policy set updated');
    });
  }
  ...
  delete(event: any) {
    ...
    this.dialogService
      ...
      .pipe(
        filter((res) => res),
        switchMap(() => this.policySetService.delete(this.domainId, this.policySet.id)),
        tap(() => {
  ...
  restoreVersion(version: any) {
    this.dialogService
      ...
      .pipe(
        filter((res) => res),
        switchMap(() => this.policySetService.restoreVersion(this.domainId, this.policySet.id, version.version)),
        tap((data) => {
          this.policySet = data;
          this.loadLatestVersion();
          this.loadVersions();
