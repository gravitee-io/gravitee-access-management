...
export class OpenFGAComponent implements OnInit, OnDestroy {
          ...
          next: (response) => {
            ...
            if (response && response.data && Array.isArray(response.data)) {
              const models = response.data.map((model: any) => {
                try {
                  const dsl = transformer.transformJSONStringToDSL(JSON.stringify(model));
                  return {
                    id: model.id,
                    dsl: dsl,
                    json: model
  ...
  private prefetchNextModelPage(token: string) {
    this.subscriptions.add(
      this.openFGAService.listAuthorizationModels(this.domainId, this.engineId, this.modelPageSize, token).subscribe({
        next: (response) => {
          const prefetchedData = (response.data || []).map((model: any) => {
            try {
              const dsl = transformer.transformJSONStringToDSL(JSON.stringify(model));
              return {
                id: model.id,
  ...
  nextPage() {
    ...
    try {
      this.tuplePagination.nextPage(this.tuplePageSize, (token) =>
        this.openFGAService.listTuples(this.domainId, this.engineId, this.tuplePageSize, token)
      ).subscribe({
        next: (response) => {
          const tuples = response.data || [];
          const token = response.info?.continuationToken || null;
  ...
  previousPage() {
    this.tuplePagination.previousPage((token) =>
      this.openFGAService.listTuples(this.domainId, this.engineId, this.tuplePageSize, token)
    ).subscribe({
      next: (response) => {
        const tuples = response.data || [];
  ...
  nextModelPage() {
    ...
    try {
      this.modelPagination.nextPage(this.modelPageSize, (token) =>
  ...
  previousModelPage() {
    this.modelPagination.previousPage((token) =>
      this.openFGAService.listAuthorizationModels(this.domainId, this.engineId, this.modelPageSize, token)
    ).subscribe({
      next: (response) => {
        const models = (response.data || []).map((model: any) => {
