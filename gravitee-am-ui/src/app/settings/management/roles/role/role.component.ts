...
export class ManagementRoleComponent implements OnInit {
  ...
  private initPermissionsDatasource() {
    this.allPermissions = new MatTableDataSource(
      this.role.availablePermissions.map((p) => ({
        permission: p,
    ...
    this.role.permissions = this.allPermissions.data
      .map((p) => {
        return Object.keys(p)
          .filter((key) => key !== 'permission' && p[key] === true)
          .map((type) => `${p.permission}_${type}`);
      })
  ...
  initBreadcrumb() {
    this.breadcrumbService.addFriendlyNameForRouteRegex(
      '/settings/management/roles/' + this.role.id + '(:?.*)?$',
