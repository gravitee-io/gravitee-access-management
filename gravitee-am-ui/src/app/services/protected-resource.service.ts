...
export class ProtectedResourceService {
  ...
  findByDomain(domainId: string, type: ProtectedResourceType, page: number, size: number): Observable<Page<ProtectedResource>> {
    return this.http.get<Page<ProtectedResource>>(this.baseURL + domainId + `/protected-resources?type=${type}&page=${page}&size=${size}`);
  }
