...
export class OpenFGAService {
  ...
  /**
   * Get authorization model for the configured store
   * Uses the plugin resource endpoint: /authorization-engines/{engineId}/authorization-model
   */
  ...
  /**
   * List tuples from the configured store
   * Uses the plugin resource endpoint: /authorization-engines/{engineId}/tuples
   */
  listTuples(domainId: string, engineId: string): Observable<any[]> {
    return this.http.get<any[]>(`${this.domainsURL}${domainId}/authorization-engines/${engineId}/settings/tuples`);
  }

  /**
   * Add a tuple to the configured store
   * Uses the plugin resource endpoint: /authorization-engines/{engineId}/tuples
   */
  ...
  /**
   * Delete a tuple from the configured store
   * Uses the plugin resource endpoint: /authorization-engines/{engineId}/tuples
   */
  ...
  /**
   * Check permission using the configured store
   * Uses the plugin resource endpoint: /authorization-engines/{engineId}/check
   */
