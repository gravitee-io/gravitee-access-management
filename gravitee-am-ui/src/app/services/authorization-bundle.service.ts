...
@Injectable()
export class AuthorizationBundleService {
  private baseURL = AppConfig.settings.domainBaseURL;

  constructor(private http: HttpClient) {}

  findByDomain(domainId: string): Observable<any> {
    return this.http.get<any>(this.baseURL + domainId + '/authorization/bundles');
  }

  get(domainId: string, bundleId: string): Observable<any> {
    return this.http.get<any>(this.baseURL + domainId + '/authorization/bundles/' + bundleId);
  }

  create(domainId: string, bundle: any): Observable<any> {
    return this.http.post<any>(this.baseURL + domainId + '/authorization/bundles', bundle);
  }

  update(domainId: string, bundleId: string, bundle: any): Observable<any> {
    return this.http.put<any>(this.baseURL + domainId + '/authorization/bundles/' + bundleId, bundle);
  }
