...
export class SidenavSettingsComponent implements OnInit {
  ...
  constructor(private router: Router, private currentRoute: ActivatedRoute, private authGuard: AuthGuard) {}

  ngOnInit() {
    const currentSnapshot = this.currentRoute.snapshot;
    this.router.config
      .filter((r) => r.path === this.filterLvl1)
      .forEach((r) => {
        r.children
          .filter((r) => r.path === this.filterLvl2)
          .forEach((r) => {
            if (this.filterLvl3) {
              r.children
                .filter((r) => r.path === this.filterLvl3)
                .forEach((r) => {
  ...
  private setPaths(currentRoute, currentSnapshot) {
    currentRoute.children
      .filter((r) => r.data && r.data.menu && this.authGuard.canDisplay(currentSnapshot, r))
      .forEach((r) => {
