...
describe('MaterialMultiselectComponent', () => {
  ...
  it('should call jsf.updateArrayCheckboxList when selected changes', () => {
    ...
    expect(jsf.updateArrayCheckboxList).toHaveBeenCalledWith(component, [{ name: 'A', value: 'A', checked: true }]);
  });
  ...
  it('matchesFilter should return true when item contains search text', () => {
    component.searchText = 'test';
    expect(component.matchesFilter('my test item')).toBe(true);
  });

  it('matchesFilter should return false when item does not match', () => {
    component.searchText = 'notfound';
    expect(component.matchesFilter('hello')).toBe(false);
  });

  it('matchesFilter should be case insensitive', () => {
    component.searchText = 'HELLO';
    expect(component.matchesFilter('heLLo world')).toBe(true);
  });
});
