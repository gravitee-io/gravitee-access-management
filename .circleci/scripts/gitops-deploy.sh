...
update_yaml_content() {
    ...
    if yq eval ".am.imageTag = \"${TAG}\"" -i "$VALUES_FILE"; then
        # Validate anchor is still present (accounting for optional whitespace)
        if ! grep -q -E "imageTag:[[:space:]]*&imageTag" "$VALUES_FILE"; then
...
# Using GitHub's documented public SSH key to prevent MITM attacks
