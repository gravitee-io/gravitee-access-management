...
public class IdentityProviderManagerImpl implements IdentityProviderManager, InitializingBean, EventListener<IdentityProviderEvent, Payload>, InMemoryIdentityProviderListener {
     ...
     */
    private final Set<String> transientProviders = ConcurrentHashMap.newKeySet();
    ...
    @Override
    public Completable registerAuthenticationProvider(IdentityProvider provider) {
        return updateAuthenticationProvider(provider)
                .doOnComplete(() -> transientProviders.add(provider.getId()));
    }
    ...
    private Completable updateAuthenticationProvider(IdentityProvider identityProvider) {
        ...
        return pluginLicenseGate.checkPersisted(
                        new Reference(identityProvider.getReferenceType(), identityProvider.getReferenceId()),
                        PluginLicenseGate.TYPE_IDENTITY_PROVIDER,
