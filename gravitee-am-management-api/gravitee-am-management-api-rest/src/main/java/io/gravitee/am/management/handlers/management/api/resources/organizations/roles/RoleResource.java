...
public class RoleResource extends AbstractResource {
    ...
    public void get(
        ...
        checkPermission(ReferenceType.ORGANIZATION, organizationId, Permission.ORGANIZATION_ROLE, Acl.READ)
            .andThen(roleService.findById(ReferenceType.ORGANIZATION, organizationId, role).map(this::convert))
    ...
    public void update(
        ...
        checkPermission(ReferenceType.ORGANIZATION, organizationId, Permission.ORGANIZATION_ROLE, Acl.UPDATE)
            .andThen(roleService.update(ReferenceType.ORGANIZATION, organizationId, role, updateRole, authenticatedUser).map(this::convert))
            .subscribe(response::resume, response::resume);
    }

    @DELETE
    @ApiOperation(
        value = "Delete a plaform role",
