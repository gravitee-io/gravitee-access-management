...
public abstract class AbstractResource {
    ...
    protected Completable checkAnyPermission(
        ...
        return checkPermissions(
            or(
                of(ReferenceType.APPLICATION, applicationId, permission, acls),
    ...
    protected Completable checkPermissions(PermissionAcls permissionAcls) {
        return hasPermission(getAuthenticatedUser(), permissionAcls).flatMapCompletable(this::checkPermission);
    }

    protected Single<Boolean> hasPermission(
        User user,
    ...
    protected Single<Boolean> hasAnyPermission(
        User user,
        String organizationId,
        String environmentId,
        String domainId,
    ...
    protected Boolean hasAnyPermission(Map<ReferenceType, Map<Permission, Set<Acl>>> permissions, Permission permission, Acl acl) {
        return (
            hasPermission(permissions.getOrDefault(ReferenceType.APPLICATION, emptyMap()), permission, acl) ||
    ...
    protected Single<Map<ReferenceType, Map<Permission, Set<Acl>>>> findAllPermissions(
        User user,
        String organizationId,
        String environmentId,
        String domainId
    ...
    protected Single<Map<ReferenceType, Map<Permission, Set<Acl>>>> findAllPermissions(
        ...
        permissionObs.add(
            applicationId != null
                ? permissionService.findAllPermissions(user, ReferenceType.APPLICATION, applicationId)
                : Single.just(emptyMap())
        );
        permissionObs.add(
            domainId != null ? permissionService.findAllPermissions(user, ReferenceType.DOMAIN, domainId) : Single.just(emptyMap())
        );
        permissionObs.add(
            environmentId != null
                ? permissionService.findAllPermissions(user, ReferenceType.ENVIRONMENT, environmentId)
                : Single.just(emptyMap())
        );
        permissionObs.add(
            organizationId != null
                ? permissionService.findAllPermissions(user, ReferenceType.ORGANIZATION, organizationId)
                : Single.just(emptyMap())
        );
