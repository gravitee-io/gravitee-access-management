...
public class ApplicationsResource extends AbstractResource {
                        ...
                        pagedApplications ->
                            Maybe
                                ...
                                .map(
                                    applications ->
                                        new Page<>(
                                            applications.stream().skip(page * size).limit(size).collect(Collectors.toList()),
                                            pagedApplications.getCurrentPage(),
                                            applications.size()
                                        )
                                )
                    )
    ...
    public void createApplication(
        ...
        @PathParam("environmentId") String environmentId,
        @PathParam("domain") String domain,
        @ApiParam(name = "application", required = true) @Valid @NotNull final NewApplication newApplication,
        @Suspended final AsyncResponse response
    ) {
        final User authenticatedUser = getAuthenticatedUser();
            ...
            .andThen(
                domainService
                    ...
                    .flatMapSingle(
                        __ ->
                            applicationService
                                .create(domain, newApplication, authenticatedUser)
                                .map(
                                    application ->
                                        Response
                                            .created(
