 ...
 */
@Tag(name = "policy set")
public class PolicySetsResource extends AbstractResource {
    ...
    @Operation(
            ...
            summary = "List registered policy sets for a security domain",
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "List registered policy sets for a security domain",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = PolicySet.class)))),
    ...
    public void list(
        ...
        checkAnyPermission(organizationId, environmentId, domainId, Permission.DOMAIN_AUTHORIZATION_BUNDLE, Acl.LIST)
                .andThen(domainService.findById(domainId)
                        ...
                        .flatMapPublisher(__ -> policySetService.findByDomain(domainId))
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Policy set successfully created",
                    content = @Content(mediaType = "application/json",
    ...
    public void create(
            ...
            @PathParam("environmentId") String environmentId,
            @PathParam("domain") String domainId,
            @Parameter(name = "policySet", required = true)
            @Valid @NotNull final NewPolicySet newPolicySet,
            @Suspended final AsyncResponse response) {
        ...
        checkAnyPermission(organizationId, environmentId, domainId, Permission.DOMAIN_AUTHORIZATION_BUNDLE, Acl.CREATE)
                .andThen(domainService.findById(domainId)
                        .switchIfEmpty(Maybe.error(new DomainNotFoundException(domainId)))
                        .flatMapSingle(existingDomain -> policySetService.create(existingDomain, newPolicySet, authenticatedUser))
                        .map(ps -> Response.created(URI.create("/organizations/" + organizationId + "/environments/"
                                                    + environmentId + "/domains/" + domainId + "/authorization/policy-sets/" + ps.getId()))
                                            .entity(ps)
                                            .build()
                        ))
                .subscribe(response::resume, response::resume);
    ...
    public PolicySetResource getPolicySetResource() {
        return resourceContext.getResource(PolicySetResource.class);
    }
}
