...
public class BulkOperationResult<T> {
    ...
    private final static int NO_INDEX = Integer.MIN_VALUE;
    ...
    public static <R> BulkOperationResult<R> error(Response.Status statusCode, Object errorDetails) {
        return new BulkOperationResult<>(NO_INDEX, statusCode, null, errorDetails);
    }

    public static <R> BulkOperationResult<R> error(Response.Status statusCode) {
        return error(statusCode, (Object) null);
    }

    public static <R> BulkOperationResult<R> error(Response.Status statusCode, Exception exception) {
        return error(statusCode, Map.of("error", exception.getClass().getSimpleName(), "error_details", exception.getMessage()));
    }
