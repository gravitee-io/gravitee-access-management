...
public class ClientsResource extends AbstractResource {
    ...
    @ApiOperation(
        value = "List registered clients for a security domain",
    ...
    @ApiResponses(
        {
            @ApiResponse(
                code = 200,
                message = "List registered clients for a security domain",
        ...
        checkAnyPermission(organizationId, environmentId, domainId, Permission.APPLICATION, Acl.LIST)
            .andThen(
                domainService
                    ...
                    .flatMapSingle(
                        domain ->
                            getClients(domainId, query, 0, Integer.MAX_VALUE)
                                .flatMap(
                                    pagedClients ->
                                        Maybe
                                            .concat(
                                            ...
                                            .map(
                                                clients -> {
                                                    List<ClientListItem> sortedClients = clients
                                                        .stream()
                                                        .map(clientEnhancer.enhanceClient(Collections.singletonMap(domainId, domain)))
                                                        .sorted(
                                                            (o1, o2) ->
                                                                String.CASE_INSENSITIVE_ORDER.compare(o1.getClientId(), o2.getClientId())
                                                        )
                                                        .collect(Collectors.toList());
                                                    ...
                                                    return new Page<>(
                                                        sortedClients.stream().skip(page * size).limit(size).collect(Collectors.toList()),
                                                        pagedClients.getCurrentPage(),
                                                        sortedClients.size()
                                                    );
                                                }
                                            )
    ...
    @ApiResponses(
        { @ApiResponse(code = 201, message = "Client successfully created"), @ApiResponse(code = 500, message = "Internal server error") }
    )
    public void createClient(
        @PathParam("organizationId") String organizationId,
        @PathParam("environmentId") String environmentId,
        @PathParam("domain") String domain,
        @ApiParam(name = "client", required = true) @Valid @NotNull final NewClient newClient,
        @Suspended final AsyncResponse response
    ) {
        final User authenticatedUser = getAuthenticatedUser();
            ...
            .andThen(
                domainService
                    ...
                    .flatMapSingle(
                        irrelevant ->
                            clientService
                                .create(domain, newClient, authenticatedUser)
                                .map(
                                    client ->
                                        Response
                                            .created(
