...
public class ReportersPluginResource {
    ...
    @ApiOperation(value = "List reporter plugins", notes = "There is no particular permission needed. User must be authenticated.")
    public void list(@Suspended final AsyncResponse response) {
        reporterPluginService
            .findAll()
            .map(
                reporterPlugins ->
                    reporterPlugins.stream().sorted(Comparator.comparing(ReporterPlugin::getName)).collect(Collectors.toList())
            )
