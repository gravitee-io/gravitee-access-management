...
public class BulkPayloadSizeFilter implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
        if (requestContext.getUriInfo().getRequestUri().getPath().contains("users/bulk")
                && requestContext.getLength() > userBulkConfiguration.bulkMaxRequestLength()) {
            throw TooManyOperationsException.payloadLimitReached(userBulkConfiguration.bulkMaxRequestLength());
        }
    }
}
