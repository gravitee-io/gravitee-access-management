...
public class RolesResource extends AbstractResource {
    ...
    @ApiOperation(
        value = "List registered roles of the organization",
    ...
    @ApiResponses(
        {
            @ApiResponse(
                code = 200,
                message = "List registered roles of the organization",
                response = RoleEntity.class,
    ...
    public void list(
        ...
        checkPermission(ReferenceType.ORGANIZATION, organizationId, Permission.ORGANIZATION_ROLE, Acl.LIST)
            .andThen(
                roleService
                    .findAllAssignable(ReferenceType.ORGANIZATION, organizationId, type)
    ...
    @ApiOperation(
        value = "Create a role for the organization",
        notes = "User must have the ORGANIZATION_ROLE[CREATE] permission on the specified organization"
    )
    @ApiResponses(
        { @ApiResponse(code = 201, message = "Role successfully created"), @ApiResponse(code = 500, message = "Internal server error") }
    )
    public void create(
        @PathParam("organizationId") String organizationId,
        ...
        checkPermission(ReferenceType.ORGANIZATION, organizationId, Permission.ORGANIZATION_ROLE, Acl.CREATE)
            .andThen(
                roleService
                    .create(ReferenceType.ORGANIZATION, organizationId, newRole, authenticatedUser)
                    .map(
                        role ->
                            Response.created(URI.create("/organizations/" + organizationId + "/roles/" + role.getId())).entity(role).build()
                    )
            )
            .subscribe(response::resume, response::resume);
    }
