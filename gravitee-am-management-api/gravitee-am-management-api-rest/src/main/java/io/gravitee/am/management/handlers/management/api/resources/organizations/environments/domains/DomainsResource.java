...
public class DomainsResource extends AbstractResource {
    ...
    @ApiResponses(
        {
            @ApiResponse(
                code = 200,
                message = "List accessible security domains for current user",
    ...
    public void list(
        ...
        checkAnyPermission(organizationId, environmentId, Permission.DOMAIN, Acl.LIST)
            .andThen(
                domainService
                    .findAllByEnvironment(organizationId, environmentId)
                    .flatMapMaybe(
                        domain ->
                            hasPermission(
                                authenticatedUser,
                                or(
                                    of(ReferenceType.DOMAIN, domain.getId(), Permission.DOMAIN, Acl.READ),
    ...
    @ApiResponses(
        { @ApiResponse(code = 201, message = "Domain successfully created"), @ApiResponse(code = 500, message = "Internal server error") }
    )
    public void create(
        @PathParam("organizationId") String organizationId,
        @PathParam("environmentId") String environmentId,
        ...
        checkAnyPermission(organizationId, environmentId, Permission.DOMAIN, Acl.CREATE)
            .andThen(
                domainService
                    .create(organizationId, environmentId, newDomain, authenticatedUser)
                    // create default idp
                ...
                domain ->
                    response.resume(
                        Response
                            .created(
                                URI.create(
                                    "/organizations/" + organizationId + "/environments/" + environmentId + "/domains/" + domain.getId()
                                )
                            )
                            .entity(domain)
                            .build()
