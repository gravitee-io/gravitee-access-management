 ...
 */
package io.gravitee.am.management.standalone.boostrap;
...
public class Bootstrap {
    ...
    private final static String CONTAINER_CLASS = "io.gravitee.am.management.standalone.container.Container";
    ...
    private void checkInstallRoot(File graviteeHomeDir) {
        ...
        if (!graviteeHomeDir.isDirectory()) {
            throw new RuntimeException("Invalid Gravitee AM Management API Standalone Home. Not a directory: "
                    + graviteeHomeDir.getAbsolutePath());
        }
        ...
        File [] files = graviteeLibDir.listFiles(pathname -> {
            return pathname.getName().startsWith("gravitee-am-management-api-standalone-bootstrap");
        });

        if (files == null || files.length == 0 || files.length > 1) {
            throw new RuntimeException("Invalid Gravitee AM Management API Standalone Home. No bootstrapable jar can be " +
