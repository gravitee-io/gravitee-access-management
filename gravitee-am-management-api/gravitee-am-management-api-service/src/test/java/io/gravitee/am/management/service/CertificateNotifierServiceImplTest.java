...
public class CertificateNotifierServiceImplTest {
    ...
    public void shouldNotifyUser_EmailOnly() throws Exception {
        ...
        doAnswer(invocation -> { latch.countDown(); return null; }).when(notifierService).register(any(), any(), any());
    ...
    public void shouldNotNotifyIfGroupIsEmpty() throws Exception {
        CountDownLatch latch = new CountDownLatch(1);
        when(domainOwnersProvider.retrieveDomainOwners(eq(domain))).thenAnswer(invocation -> {
            latch.countDown();
