...
public class DefaultOrganizationUpgraderTest {
    ...
    public void before() {
        cut =
            new DefaultOrganizationUpgrader(
                organizationService,
    ...
    public void shouldCreateDefaultOrganization() {
        ...
        when(
            identityProviderService.create(
                ...
                any(NewIdentityProvider.class),
                isNull()
            )
        )
            .thenReturn(Single.just(idp));
        when(organizationService.update(eq(Organization.DEFAULT), any(PatchOrganization.class), isNull()))
            .thenReturn(Single.just(organization));
        when(
            userService.create(
                argThat(
                    user ->
                        ...
                        user.getReferenceId().equals(Organization.DEFAULT)
                )
            )
        )
            .thenReturn(Single.just(adminUser));
        ...
        when(identityProviderService.findAll(ReferenceType.ORGANIZATION, Organization.DEFAULT))
    ...
    public void shouldNotUpdateIdentityProviderRoleMapper_noInlineIdp() {
        ...
        when(identityProviderService.findAll(ReferenceType.ORGANIZATION, Organization.DEFAULT))
            .thenReturn(Single.just(Collections.singletonList(idp)));
    ...
    public void shouldCreateAdminUser_noAdminUser() {
        ...
        when(identityProviderService.findAll(ReferenceType.ORGANIZATION, Organization.DEFAULT))
            .thenReturn(Single.just(Collections.singletonList(idp)));
        when(userService.findByUsernameAndSource(ReferenceType.ORGANIZATION, Organization.DEFAULT, "admin", idp.getId()))
    ...
    public void shouldNotUpdateAdminUser_adminAlreadyExist() {
        ...
        when(identityProviderService.findAll(ReferenceType.ORGANIZATION, Organization.DEFAULT))
            .thenReturn(Single.just(Collections.singletonList(idp)));
        when(userService.findByUsernameAndSource(ReferenceType.ORGANIZATION, Organization.DEFAULT, "admin", idp.getId()))
            .thenReturn(Maybe.just(adminUser)); // Admin already exists.
        when(organizationService.createDefault()).thenReturn(Maybe.empty());
