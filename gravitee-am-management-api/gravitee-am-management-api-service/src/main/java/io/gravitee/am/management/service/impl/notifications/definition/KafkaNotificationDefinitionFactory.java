...
public class KafkaNotificationDefinitionFactory<T extends NotifierSubject> implements NotificationDefinitionFactory<T> {
    ...
    public Maybe<NotificationDefinition> buildNotificationDefinition(T object) {
        try {
            ...
            data.put("template", templateProvider.getNotificationTemplate(notifierSettings.template().template()));
            data.put("type", notifierSettings.type());
            data.put("metadata", object.getMetadata());

            final NotificationDefinition definition = new NotificationDefinition();
            definition.setType(TYPE_KAFKA_NOTIFIER);
            definition.setConfiguration(mapper.writeValueAsString(kafkaNotifierConfiguration));
