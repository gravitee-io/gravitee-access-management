...
/**
 * Moves the fetch, SSRF and cache limits a security domain persisted under its SPIFFE settings into
 * the neutral block that now governs key retrieval for every trusted domain.
 *
...
public class DomainKeyRetrievalSettingsUpgrader extends AsyncUpgrader {
    ...
    private Maybe<Domain> upgradeDomain(Domain domain) {
        ...
        KeyRetrievalSettings relocated = oidc.getKeyRetrievalSettings();
        oidc.setKeyRetrievalSettings(relocated);
