                                ...
                                return identityProviderManager.getUserProvider(user.getSource())
                                        ...
                                        .flatMap(userProvider -> {
                                            ...
                                            return passwordPolicyService.retrievePasswordPolicy(user, client, identityProvider.orElse(null))
                                                    .map(Optional::ofNullable)
                                                    .switchIfEmpty(Maybe.just(Optional.empty()))
                                                    .flatMapSingle(optPolicy -> ensurePasswordMatchesPolicy(password, user, optPolicy)
                                                            .flatMap(ignore -> passwordHistoryService.addPasswordToHistory(DOMAIN, domain.getId(), user, password, principal, optPolicy.orElse(null)).toSingle()))
                                                    .ignoreElement()
