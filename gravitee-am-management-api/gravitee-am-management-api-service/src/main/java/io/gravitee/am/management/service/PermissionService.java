...
public class PermissionService {
    ...
    public PermissionService(MembershipService membershipService, GroupService groupService, RoleService roleService) {
            ...
            .flatMap(
                userGroupIds -> {
                    ...
                    return Flowable
                        .merge(
                            referenceStream
                                .map(p -> membershipService.findByCriteria(p.getKey(), p.getValue(), criteria))
                        ...
                        .flatMap(
                            allMemberships -> {
                                ...
                                // Get all roles.
                                return roleService
                                    .findByIdIn(allMemberships.stream().map(Membership::getRoleId).collect(Collectors.toList()))
    ...
    private Map<Membership, Map<Permission, Set<Acl>>> permissionsPerMembership(List<Membership> allMemberships, Set<Role> allRoles) {
        ...
        Map<Membership, Map<Permission, Set<Acl>>> rolesPerMembership = allMemberships
            .stream()
