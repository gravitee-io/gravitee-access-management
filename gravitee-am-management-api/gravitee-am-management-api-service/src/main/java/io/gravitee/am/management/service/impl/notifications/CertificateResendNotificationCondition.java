...
public class CertificateResendNotificationCondition implements ResendNotificationCondition {
    ...
    private int dayBeforeResend;

    public CertificateResendNotificationCondition(int dayBeforeResend) {
        this.dayBeforeResend = dayBeforeResend;
    }
    ...
    public Boolean apply(NotificationDefinition definition, NotificationAcknowledge notificationAcknowledge) {
        return notificationAcknowledge.getUpdatedAt() != null &&
                Instant.ofEpochMilli(notificationAcknowledge.getUpdatedAt().getTime()).plus(dayBeforeResend, ChronoUnit.DAYS).isBefore(Instant.now());
    }
}
