...
public class AuthorizationEnginePluginManager
    ...
    public AuthorizationEngineProvider create(ProviderConfiguration providerConfig) {
        var authEngine = getAuthorizationEngineOrThrow(providerConfig);
    ...
    private AuthorizationEngine<?, AuthorizationEngineProvider> getAuthorizationEngineOrThrow(ProviderConfiguration providerConfig) {
        logger.debug("Looking for an authorization engine for [{}]", providerConfig.getType());
        return Optional.ofNullable(get(providerConfig.getType()))
                .orElseThrow(() -> new IllegalStateException("No authorization engine is registered for type " + providerConfig.getType()));
    }
