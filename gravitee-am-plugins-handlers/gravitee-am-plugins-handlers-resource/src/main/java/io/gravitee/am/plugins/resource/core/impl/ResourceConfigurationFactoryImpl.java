...
public class ResourceConfigurationFactoryImpl implements ResourceConfigurationFactory {
    ...
    @Override
    public <T extends ResourceConfiguration> T create(Class<T> clazz, String content) {
        logger.debug("Create a new instance of resource configuration for class: {}", clazz.getName());

        try {
            return objectMapper.readValue(content, clazz);
        } catch (IOException ioe) {
            logger.error("Unable to create an resource configuration", ioe);
            return null;
        }
    }
}
